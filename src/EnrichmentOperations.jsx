...
import{Activity,AlertTriangle,BookOpen,CheckCircle2,Clock3,DollarSign,RefreshCw,ShieldCheck,TrendingUp,Workflow}from'lucide-react'
 ...
 return <section className="eops" data-cf245-enrichment-operations="true">
  ...
  <div className="eops-grid">
   ...
   <article className="eops-panel eops-wide"><Panel title="Hourly enrichment funnel" subtitle="Acquisition and deterministic extraction are not reported as published enrichment."/>
    <div className="eops-table-wrap"><table><thead><tr><th>Hour</th><th>Country</th><th>Items</th><th>Fetched</th><th>Evidence</th><th>Extraction</th><th>URLs found</th><th>Intakes</th><th>English</th><th>Tuition</th><th>L3</th><th>429</th><th>5xx</th><th>p50 / p95 response</th></tr></thead><tbody>{hourly.slice(-24).reverse().map((x,i)=><tr key={`${x.hour_utc}-${x.country_code}-${x.domain}-${i}`}><td>{when(x.hour_utc)}</td><td>{x.country_code||'—'}</td><td>{n(x.items)}</td><td>{n(x.fetched)}</td><td>{n(x.evidence_created)}</td><td>{n(x.extraction_attempted)}</td><td>{n(x.official_urls_found)}</td><td>{n(x.intakes_found)}</td><td>{n(x.english_requirements_found)}</td><td>{n(x.provider_current_tuition_found)}</td><td>{n(x.layer3_escalated)}</td><td>{n(x.http_429)}</td><td>{n(x.http_5xx)}</td><td>{ms(x.p50_response_ms)} / {ms(x.p95_response_ms)}</td></tr>)}</tbody></table></div>
