...
export function JobsWorkspace(){
  ...
  // Package 5 (J1, J4): show count columns only when a job on this page recorded something.
  const hasCounts=keys=>rows.some(r=>keys.some(k=>r[k]!=null&&Number(r[k])!==0))
  const showRecords=hasCounts(['discovered_count','selected_count','processed_count','accepted_count','rejected_count']),showChanges=hasCounts(['creates_count','updates_count','unchanged_count','conflicts_count']),colCount=8+(showRecords?1:0)+(showChanges?1:0)
