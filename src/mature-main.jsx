...
function DetailBody({type,data,navigate}){if(!data)return <EmptyInline text="No detail returned."/>;const scalars=Object.entries(data).filter(([,v])=>v==null||['string','number','boolean'].includes(typeof v)).slice(0,24);return <div className="m-drawer-body"><div className="m-detail-grid">{scalars.map(([k,v])=><div key={k}><small>{humanise(k)}</small><strong>{formatScalar(k,v)}</strong></div>)}</div>{type==='course'&&<CourseSemantics data={data} navigate={navigate}/>}<ObjectSections data={data} navigate={navigate} exclude={type==='course'?['fee_summary','entry_summary','taxonomy_summary','state_summary']:[]}/></div>}
