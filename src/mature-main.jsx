 ...
 return <div className="m-page-stack">
  <section className="m-panel">
   ...
   <div className="m-stats-grid">
    ...
    <article className="m-stats-card"><span>QS World University Rankings</span><strong>{qs?.latest_edition||'2027'}</strong><small>{qs?.accepted_editions?`${Number(qs.observations||0).toLocaleString()} observations · ${Number(qs.mapped_observations||0).toLocaleString()} mapped`:'Source configured · no accepted edition applied yet.'}</small><div>{qs?.accepted_editions?<><button onClick={()=>navigate('Compare',{type:'provider'})}>Compare</button><button onClick={()=>navigate('Administration',{section:'sources-imports',system:'qs_wur',year:'2027'})}>Manage imports</button></>:<button onClick={()=>navigate('Administration',{section:'sources-imports',system:'qs_wur',year:'2027'})}>Manage imports</button>}</div></article>
    <article className="m-stats-card"><span>Times Higher Education</span><strong>{the?.latest_edition||'2026'}</strong><small>{the?.accepted_editions?`${Number(the.observations||0).toLocaleString()} observations · ${Number(the.mapped_observations||0).toLocaleString()} mapped`:'Source configured · no accepted edition applied yet.'}</small><div>{the?.accepted_editions?<><button onClick={()=>navigate('Compare',{type:'provider'})}>Compare</button><button onClick={()=>navigate('Administration',{section:'sources-imports',system:'the_wur',year:'2026'})}>Manage imports</button></>:<button onClick={()=>navigate('Administration',{section:'sources-imports',system:'the_wur',year:'2026'})}>Manage imports</button>}</div></article>
   </div>
  </section>
  <section className="m-panel">
