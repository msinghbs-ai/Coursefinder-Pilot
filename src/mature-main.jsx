...
function RankingImportPanel({onError,routeParams,navigate}){
 ...
 async function processImport(importId,action='validate'){
  ...
  try{
   ...
   if(action==='validate')setSaved('Evidence parsed and validated. Review reconciliation, then Apply edition when ready.')
 ...
 return <div className="m-page-stack m-ranking-import-page">
  ...
  <section className="m-panel m-ranking-import-history">
   <div className="m-ranking-history-head"><div><h3>Ranking import workflow</h3><p>Acquire Evidence → Parse & validate → Apply edition → Review exceptions. All editions remain visible here.</p></div><div className="m-ranking-import-row-actions"><label className="m-compact-filter">Publisher<select value={historySystem} onChange={e=>setHistorySystem(e.target.value)}><option value="all">All</option><option value="qs_wur">QS</option><option value="the_wur">THE</option><option value="arwu">ARWU</option></select></label><button className="m-link-button" disabled={processingId===x.id} onClick={async()=>{try{const{data,error}=await supabase.functions.invoke('ranking-evidence-export',{body:{system_code:x.system_code,edition_year:Number(x.edition_year),import_id:x.id}});if(error)throw error;if(!data?.signed_url)throw new Error('Exportable XLSX Evidence not found');const a=document.createElement('a');a.href=data.signed_url;a.download=data.filename||'';a.rel='noopener';document.body.appendChild(a);a.click();a.remove()}catch(e){setSaved(`Export failed: ${e.message||String(e)}`)}}}>Export XLSX</button><button className="m-secondary compact" onClick={()=>navigate?.('Jobs')}><Workflow size={13}/>Jobs</button></div></div>
