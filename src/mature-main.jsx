...
function DetailBody({type,data,navigate}){if(!data)return <EmptyInline text="No detail returned."/>;if(type==='course')return <CourseDetailPolish data={data} navigate={navigate}/>;const scalars=Object.entries(data).filter(([,v])=>v==null||['string','number','boolean'].includes(typeof v)).slice(0,24);return <div className="m-drawer-body"><div className="m-detail-grid">{scalars.map(([k,v])=><div key={k}><small>{humanise(k)}</small><strong>{formatScalar(k,v)}</strong></div>)}</div><ObjectSections data={data} navigate={navigate}/></div>}
