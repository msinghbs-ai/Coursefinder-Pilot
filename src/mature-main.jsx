...
function FilterBar({type,filters,patch,data,busy,advanced}){
  ...
  return <div className={`m-filter-bar m-course-filters ${advanced?'expanded':''}`}>
    <PagedFilterSelect kind="country" label="Country" value={filters.country||''} onChange={(v,l)=>patch('country',v,l)}/>
    <PagedFilterSelect kind="subdivision" label="State / Region" value={filters.subdivision||''} country={filters.country||''} onChange={(v,l)=>patch('subdivision',v,l)}/>
    <PagedFilterSelect kind="provider" label="Provider" value={filters.provider||''} country={filters.country||''} subdivision={filters.subdivision||''} onChange={(v,l)=>patch('provider',v,l)}/>
    <PagedFilterSelect kind="level" label="Study level" value={filters.level||''} country={filters.country||''} subdivision={filters.subdivision||''} onChange={(v,l)=>patch('level',v,l)}/>
    {advanced&&<><PagedFilterSelect kind="field" label="Field" value={filters.field||''} country={filters.country||''} subdivision={filters.subdivision||''} onChange={(v,l)=>patch('field',v,l)}/><PagedFilterSelect kind="delivery" label="Delivery" value={filters.delivery||''} country={filters.country||''} subdivision={filters.subdivision||''} onChange={(v,l)=>patch('delivery',v,l)}/><TriFilter label="Has fee" value={filters.hasFee||''} onChange={(v,l)=>patch('hasFee',v,l)}/><TriFilter label="Has intake" value={filters.hasIntake||''} onChange={(v,l)=>patch('hasIntake',v,l)}/><TriFilter label="Has English" value={filters.hasEnglish||''} onChange={(v,l)=>patch('hasEnglish',v,l)}/><TriFilter label="Has scholarship" value={filters.hasScholarship||''} onChange={(v,l)=>patch('hasScholarship',v,l)}/><FilterSelect label="Min readiness" value={filters.minCompleteness||''} onChange={(v,l)=>patch('minCompleteness',v,l)} options={[50,75,90,100].map(n=>opt(String(n),`${n}%+`))}/><FilterSelect label="Freshness" value={filters.freshness||''} onChange={(v,l)=>patch('freshness',v,l)} options={[opt('never_verified','Never verified'),opt('modified_7d','Modified in 7 days'),opt('modified_30d','Modified in 30 days'),opt('stale_180d','Stale / never verified (180d)')]}/><FilterSelect label="Lifecycle" value={filters.lifecycle||''} onChange={(v,l)=>patch('lifecycle',v,l)} options={STATUS_OPTIONS}/><FilterSelect label="Publication" value={filters.publication||''} onChange={(v,l)=>patch('publication',v,l)} options={PUBLICATION_OPTIONS}/></>}
  </div>
}
