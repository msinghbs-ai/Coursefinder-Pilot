...
function pageFromHash(){const s=location.hash.replace(/^#/,'');for(const[,items]of NAV)for(const i of items)if(i.slug===s)return i.label;return'Dashboard'}

function App(){
  const[session,setSession]=useState(null),[booting,setBooting]=useState(true),[context,setContext]=useState(null)
  const[page,setPage]=useState(pageFromHash()),[error,setError]=useState(''),[navOpen,setNavOpen]=useState(false),[collapsed,setCollapsed]=useState(false)
  ...
  useEffect(()=>{const h=()=>setPage(pageFromHash());addEventListener('hashchange',h);return()=>removeEventListener('hashchange',h)},[])
  function go(label){const target=slug(label);if(location.hash!==`#${target}`)location.hash=target;else setPage(label);setNavOpen(false);requestAnimationFrame(()=>{if(mainRef.current)mainRef.current.scrollTop=0})}
  ...
  return <div className={`m-shell ${collapsed?'is-collapsed':''}`}>
    ...
    <main className="m-main" ref={mainRef}>
      ...
      <Page page={page} rank={rank} onError={setError} navigate={go}/>
    </main>
  </div>
}
...
function Page({page,rank,onError,navigate}){
  if(page==='Dashboard')return <Dashboard onError={onError} navigate={navigate}/>
  if(page==='Providers')return <Catalogue type="provider" onError={onError}/>
  if(page==='Courses')return <Catalogue type="course" onError={onError}/>
  if(page==='Campuses')return <Catalogue type="campus" onError={onError}/>
  if(page==='Scholarships')return <Catalogue type="scholarship" onError={onError}/>
  ...
  if(page==='Evidence'&&rank>=3)return <Evidence onError={onError}/>
...
function ActivityFeed({items,navigate}){if(!items.length)return <EmptyInline text="No recent governed activity is available."/>;return <div className="m-activity-list">{items.map((x,i)=>{const Icon=x.kind==='job'?Workflow:x.kind==='review'?ClipboardCheck:FileCheck2;const target=x.kind==='job'?'Jobs':x.kind==='review'?'Review Queue':'Evidence';return <button key={x.id||i} onClick={()=>navigate(target)} className="m-activity-row"><span className={`m-activity-icon kind-${x.kind}`}><Icon size={15}/></span><span className="m-activity-copy"><strong>{x.title||humanise(x.kind)}</strong><small>{x.detail||'Governed activity'} · {relativeTime(x.occurred_at)}</small></span><Status value={x.status}/></button>})}</div>}
...
function Catalogue({type,onError,completenessMode=false}){
  ...
  return <div className="m-page-stack">
    ...
    {selected&&<DetailDrawer type={type} data={detail} busy={detailBusy} onClose={()=>{setSelected(null);setDetail(null)}}/>}
  </div>
}

function FilterBar({type,filters,patch,data,busy,advanced}){
  ...
  if(type==='provider')return <div className="m-filter-bar"><FilterSelect label="Country" value={filters.country||''} onChange={v=>patch('country',v)} options={countries} loading={busy}/><FilterSelect label="State / Region" value={filters.subdivision||''} onChange={v=>patch('subdivision',v)} options={subdivisions} loading={busy}/><FilterSelect label="Lifecycle" value={filters.lifecycle||''} onChange={v=>patch('lifecycle',v)} options={STATUS_OPTIONS}/><FilterSelect label="Publication" value={filters.publication||''} onChange={v=>patch('publication',v)} options={PUBLICATION_OPTIONS}/></div>
  if(type==='campus')return <div className="m-filter-bar"><FilterSelect label="Country" value={filters.country||''} onChange={v=>patch('country',v)} options={countries} loading={busy}/><FilterSelect label="State / Region" value={filters.subdivision||''} onChange={v=>patch('subdivision',v)} options={subdivisions} loading={busy}/></div>
  if(type==='scholarship')return <div className="m-filter-bar"><FilterSelect label="Country" value={filters.country||''} onChange={v=>patch('country',v)} options={countries} loading={busy}/><FilterSelect label="Lifecycle" value={filters.lifecycle||''} onChange={v=>patch('lifecycle',v)} options={STATUS_OPTIONS}/><FilterSelect label="Publication" value={filters.publication||''} onChange={v=>patch('publication',v)} options={PUBLICATION_OPTIONS}/></div>
  if(type!=='course')return null
  const providers=(data.providers||[]).map(x=>opt(x.id,x.name,x.stable_key)),levels=(data.levels||[]).map(x=>opt(x.code,x.name,x.code)),fields=(data.fields||[]).map(x=>opt(x.code,x.name,x.code)),delivery=(data.delivery_modes||[]).map(x=>opt(x.code,humanise(x.name||x.code),x.code))
  return <div className={`m-filter-bar m-course-filters ${advanced?'expanded':''}`}>
    <FilterSelect label="Country" value={filters.country||''} onChange={v=>patch('country',v)} options={countries} loading={busy}/>
    <FilterSelect label="State / Region" value={filters.subdivision||''} onChange={v=>patch('subdivision',v)} options={subdivisions} loading={busy}/>
    <FilterSelect label="Provider" value={filters.provider||''} onChange={v=>patch('provider',v)} options={providers} loading={busy}/>
    <FilterSelect label="Study level" value={filters.level||''} onChange={v=>patch('level',v)} options={levels} loading={busy}/>
    {advanced&&<><FilterSelect label="Field" value={filters.field||''} onChange={v=>patch('field',v)} options={fields} loading={busy}/><FilterSelect label="Delivery" value={filters.delivery||''} onChange={v=>patch('delivery',v)} options={delivery} loading={busy}/><TriFilter label="Has fee" value={filters.hasFee||''} onChange={v=>patch('hasFee',v)}/><TriFilter label="Has intake" value={filters.hasIntake||''} onChange={v=>patch('hasIntake',v)}/><TriFilter label="Has English" value={filters.hasEnglish||''} onChange={v=>patch('hasEnglish',v)}/><TriFilter label="Has scholarship" value={filters.hasScholarship||''} onChange={v=>patch('hasScholarship',v)}/><FilterSelect label="Min readiness" value={filters.minCompleteness||''} onChange={v=>patch('minCompleteness',v)} options={[50,75,90,100].map(n=>opt(String(n),`${n}%+`))}/><FilterSelect label="Freshness" value={filters.freshness||''} onChange={v=>patch('freshness',v)} options={[opt('never_verified','Never verified'),opt('modified_7d','Modified in 7 days'),opt('modified_30d','Modified in 30 days'),opt('stale_180d','Stale / never verified (180d)')]}/><FilterSelect label="Lifecycle" value={filters.lifecycle||''} onChange={v=>patch('lifecycle',v)} options={STATUS_OPTIONS}/><FilterSelect label="Publication" value={filters.publication||''} onChange={v=>patch('publication',v)} options={PUBLICATION_OPTIONS}/></>}
  </div>
}
...
function FilterSelect({label,value,onChange,options=[],loading=false}){const[open,setOpen]=useState(false),[search,setSearch]=useState('');const ref=useRef(null);useEffect(()=>{const close=e=>{if(ref.current&&!ref.current.contains(e.target))setOpen(false)};addEventListener('mousedown',close);return()=>removeEventListener('mousedown',close)},[]);const selected=options.find(x=>String(x.value)===String(value));const filtered=options.filter(x=>`${x.label} ${x.meta||''}`.toLowerCase().includes(search.toLowerCase())).slice(0,250);return <div className="m-filter-select" ref={ref}><button className={`m-filter-button ${value?'has-value':''}`} onClick={()=>setOpen(x=>!x)}><span><small>{label}</small><strong>{selected?.label||'All'}</strong></span>{loading?<span className="m-spinner tiny"/>:<ChevronDown size={14}/>}</button>{open&&<div className="m-filter-popover"><div className="m-filter-search"><Search size={14}/><input autoFocus value={search} onChange={e=>setSearch(e.target.value)} placeholder={`Search ${label.toLowerCase()}…`}/></div><button className={!value?'selected':''} onClick={()=>{onChange('');setOpen(false);setSearch('')}}><span>All</span></button>{filtered.map(x=><button key={String(x.value)} className={String(value)===String(x.value)?'selected':''} onClick={()=>{onChange(x.value);setOpen(false);setSearch('')}}><span>{x.label}</span>{x.meta&&<small>{x.meta}</small>}</button>)}{!filtered.length&&<div className="m-option-empty">No matching options</div>}</div>}</div>}
function TriFilter({label,value,onChange}){return <FilterSelect label={label} value={value} onChange={onChange} options={[opt('true','Yes'),opt('false','No')]}/>}
function opt(value,label,meta=''){return{value,label,meta}}
function FilterChip({label,onRemove}){return <span className="m-filter-chip">{label}<button onClick={onRemove}><X size={12}/></button></span>}
...
function DetailDrawer({type,data,busy,onClose}){useEffect(()=>{const k=e=>e.key==='Escape'&&onClose();addEventListener('keydown',k);return()=>removeEventListener('keydown',k)},[onClose]);return <><button className="m-drawer-backdrop" onClick={onClose}/><aside className="m-drawer"><div className="m-drawer-head"><div><small>{humanise(type)} detail</small><h2>{detailTitle(data,type)}</h2></div><button onClick={onClose}><X size={18}/></button></div>{busy?<div className="m-drawer-loading"><span className="m-spinner"/>Loading governed detail…</div>:<DetailBody type={type} data={data}/>}</aside></>}
...
function DetailBody({type,data}){if(!data)return <EmptyInline text="No detail returned."/>;const scalars=Object.entries(data).filter(([,v])=>v==null||['string','number','boolean'].includes(typeof v)).slice(0,24);return <div className="m-drawer-body"><div className="m-detail-grid">{scalars.map(([k,v])=><div key={k}><small>{humanise(k)}</small><strong>{formatScalar(k,v)}</strong></div>)}</div>{type==='course'&&<CourseSemantics data={data}/>}<ObjectSections data={data} exclude={type==='course'?['fee_summary','entry_summary','taxonomy_summary','state_summary']:[]}/></div>}
function CourseSemantics({data}){const fees=data.fee_summary||{},cricos=fees.cricos_registered||[],provider=fees.provider_current||[];return <><section className="m-detail-section"><h3>Fee semantics</h3><p className="m-help">CRICOS registered total-course values are kept separate from Provider-current published fees.</p><div className="m-semantic-grid"><div><small>CRICOS registered rows</small><strong>{cricos.length}</strong>{cricos.slice(0,4).map((x,i)=><span key={i}>{humanise(x.fee_type)} · {x.currency||x.currency_code||''} {x.amount==null?'—':Number(x.amount).toLocaleString()} · {humanise(x.basis)}</span>)}</div><div><small>Provider-current rows</small><strong>{provider.length}</strong>{provider.length?provider.slice(0,4).map((x,i)=><span key={i}>{humanise(x.fee_type)} · {x.currency||x.currency_code||''} {x.amount==null?'—':Number(x.amount).toLocaleString()}</span>):<span>No Provider-current fee observation.</span>}</div></div></section>{data.state_summary&&<KeyObject title="Publication & Search state" value={data.state_summary}/>} {data.entry_summary&&<KeyObject title="Intakes & English" value={data.entry_summary}/>} {data.taxonomy_summary&&<KeyObject title="Taxonomy lineage" value={data.taxonomy_summary}/>}</>}
function ObjectSections({data,exclude=[]}){return <>{Object.entries(data).filter(([k,v])=>!exclude.includes(k)&&v&&typeof v==='object').map(([k,v])=><KeyObject key={k} title={humanise(k)} value={v}/>)}</>}
function KeyObject({title,value}){const arr=Array.isArray(value)?value:null;return <section className="m-detail-section"><h3>{title}</h3>{arr?<div className="m-record-list">{arr.length?arr.slice(0,25).map((x,i)=><Record key={x?.id||i} value={x}/>):<EmptyInline text="No records."/>}</div>:<div className="m-kv-list">{Object.entries(value||{}).slice(0,30).map(([k,v])=><div key={k}><span>{humanise(k)}</span><strong>{typeof v==='object'?summarise(v):formatScalar(k,v)}</strong></div>)}</div>}</section>}
function Record({value}){if(value==null)return null;if(typeof value!=='object')return <div className="m-record"><span>{String(value)}</span></div>;const title=value.title||value.name||value.canonical_title||value.source_label||value.evidence_type||value.status||value.code||value.id;return <div className="m-record"><strong>{title||'Record'}</strong><span>{Object.entries(value).filter(([k,v])=>k!=='id'&&v!=null&&typeof v!=='object').slice(0,4).map(([k,v])=>`${humanise(k)}: ${formatScalar(k,v)}`).join(' · ')}</span></div>}

function Completeness({onError}){return <Catalogue type="course" onError={onError} completenessMode/>}
...
function InsightWorkspace({kind,onError}){const[filters,setFilters]=useState({}),[options,setOptions]=useState({}),[data,setData]=useState(null),[busy,setBusy]=useState(false),[offset,setOffset]=useState(0),[query,setQuery]=useState('');useEffect(()=>{const p=kind==='qilt'?api.qiltFilterOptions(filters.survey||''):api.prismsFilterOptions();p.then(setOptions).catch(e=>onError(e.message))},[kind,filters.survey]);const debounced=useDebounce(query,280);useEffect(()=>{setBusy(true);const p=kind==='qilt'?api.qiltPage({limit:PAGE_SIZE,offset,query:debounced,survey:filters.survey||'',metric:filters.metric||'',provider:filters.provider||'',status:filters.status||'',year:filters.year||'',sort:'provider',direction:'asc'}):api.prismsPage({limit:PAGE_SIZE,offset,query:debounced,subdivision:filters.subdivision||'',studyArea:filters.studyArea||'',sector:filters.sector||'',remoteness:filters.remoteness||'',suppressed:filters.suppressed===''?null:filters.suppressed==='true'});p.then(setData).catch(e=>onError(e.message)).finally(()=>setBusy(false))},[kind,offset,debounced,JSON.stringify(filters)]);useEffect(()=>setOffset(0),[debounced,JSON.stringify(filters)]);const rows=rowsOf(data),total=Number(data?.total??rows.length);const defs=kind==='qilt'?[['survey','Survey',options.surveys],['metric','Metric',options.metrics],['provider','Provider',options.providers],['year','Year',options.years],['status','Status',options.statuses]]:[['subdivision','State / Region',options.subdivisions],['studyArea','Study area',options.study_areas],['sector','Sector',options.sectors],['remoteness','Remoteness',options.remoteness_areas]];return <div className="m-page-stack"><section className="m-panel"><div className="m-workspace-head"><div><h2>{kind==='qilt'?'QILT outcomes':'PRISMS student flow'}</h2><p>Structured Layer 2 observations remain separate from canonical identity.</p></div><div className="m-result-count"><strong>{fmtNumber(total)}</strong><span>matching</span></div></div><div className="m-search-row"><label className="m-searchbox"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search governed observations…"/></label></div><div className="m-filter-bar">{defs.map(([k,l,list])=><FilterSelect key={k} label={l} value={filters[k]||''} onChange={v=>setFilters(f=>({...f,[k]:v}))} options={normaliseOptions(list)}/>)}</div><DynamicTable rows={rows} loading={busy}/><Pager offset={offset} limit={PAGE_SIZE} total={total} onOffset={setOffset}/></section></div>}

function Evidence({onError}){const[query,setQuery]=useState(''),[type,setType]=useState(''),[opts,setOpts]=useState({}),[data,setData]=useState(null),[offset,setOffset]=useState(0),[busy,setBusy]=useState(false);useEffect(()=>{api.evidenceFilterOptions().then(setOpts).catch(e=>onError(e.message))},[]);const debounced=useDebounce(query,250);useEffect(()=>{setBusy(true);api.evidencePage({limit:PAGE_SIZE,offset,query:debounced,evidenceType:type,sort:'captured',direction:'desc'}).then(setData).catch(e=>onError(e.message)).finally(()=>setBusy(false))},[offset,debounced,type]);const rows=rowsOf(data),total=Number(data?.total??rows.length);return <div className="m-page-stack"><section className="m-panel"><div className="m-workspace-head"><div><h2>Evidence workspace</h2><p>Private evidence metadata, provenance and source snapshots through the governed read boundary.</p></div><div className="m-result-count"><strong>{fmtNumber(total)}</strong><span>matching</span></div></div><div className="m-search-row"><label className="m-searchbox"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search source, hash, evidence type or entity…"/></label><FilterSelect label="Evidence type" value={type} onChange={setType} options={normaliseOptions(opts.evidence_types||opts.types)}/></div><DynamicTable rows={rows} loading={busy}/><Pager offset={offset} limit={PAGE_SIZE} total={total} onOffset={setOffset}/></section></div>}
