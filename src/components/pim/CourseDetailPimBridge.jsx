...
export default function CourseDetailPimBridge({data,navigate}){
 ...
 const familyName=data.pim_family_name??data.pim?.family_name??data.pim?.family?.name??''
 const values=data.pim_attribute_values??data.attribute_values??data.pim?.attribute_values??[]
 return <><CourseDetailPolish data={data} navigate={navigate}/><DynamicPimFields entityType="course" familyId={familyId} familyName={familyName} values={Array.isArray(values)?values:[]}/></>
}
