...
function IntegrationSecrets({data,onSaved}){
 ...
 const[apollo,setApollo]=useState(''),[automation,setAutomation]=useState(''),[profileId,setProfileId]=useState(profiles[0]?.id||''),[openrouter,setOpenrouter]=useState(''),[reason,setReason]=useState('Production credential rotation'),[gateReason,setGateReason]=useState(''),[busy,setBusy]=useState(false),[err,setErr]=useState('')
 ...
 const layer3Gate=async(fn,done)=>{setBusy(true);setErr('');try{const{data:r,error}=await supabase.rpc(fn,{p_profile_id:profileId,p_reason:gateReason});if(error)throw error;if(r?.ok===false)throw new Error('Activation refused: '+(r.refused||[]).join('; '));setGateReason('');await onSaved(done)}catch(e){setErr(e.message)}finally{setBusy(false)}}
 const selectedL3=profiles.find(x=>x.id===profileId)
 ...
 return <section className="env-panel"><h3>Other integration credentials</h3><div className="env-grid">
  ...
  <article className="env-card"><strong>OpenRouter / Layer 3</strong><label>Profile<select value={profileId} onChange={e=>setProfileId(e.target.value)}>{profiles.map(p=><option key={p.id} value={p.id}>{p.code} · {p.model_identifier}</option>)}</select></label><label>API key<input type="password" value={openrouter} onChange={e=>setOpenrouter(e.target.value)} placeholder="Write-only"/></label><label>Reason<input value={reason} onChange={e=>setReason(e.target.value)}/></label><button disabled={busy||!profileId||openrouter.length<20} onClick={saveOpenRouter}>Save OpenRouter key</button>{selectedL3&&<small>State: {selectedL3.enabled===false?'Disabled':selectedL3.paused?'Paused':'ACTIVE'} · {selectedL3.credential_configured?'credential stored':'no credential'}</small>}<label>Activation / pause reason<input value={gateReason} onChange={e=>setGateReason(e.target.value)} placeholder="Why is this profile being activated or paused?"/></label><button disabled={busy||!profileId||gateReason.trim().length<8||selectedL3?.paused===false} onClick={()=>layer3Gate('layer3_profile_activate','Layer 3 profile activated.')}>Activate profile</button><button disabled={busy||!profileId||gateReason.trim().length<5||selectedL3?.paused===true} onClick={()=>layer3Gate('layer3_profile_pause','Layer 3 profile paused.')}>Pause profile</button><small>Activation re-checks a passed benchmark within 14 days, the qualified binding hash, a stored credential, and that no other profile is active for the same task. Every activation, refusal and pause is recorded. A profile pauses itself if its deployed code drifts from what was qualified.</small></article>
