...
const STATES=[['','All'],['needs_catalogue_page','Needs catalogue page'],['identity_check_running','Identity check running'],['site_limited','Site limited'],['not_yet_assessed','Not yet assessed']]
const LABEL={needs_catalogue_page:'Needs catalogue page',identity_check_running:'Identity check running',site_limited:'Site limited',not_yet_assessed:'Not yet assessed',in_progress:'In progress'}
...
export default function ProviderOnboarding({rank=0}){
  ...
  return <section className="l2o-panel l2po">
    ...
    {data&&<p className="l2po-summary"><strong>{n(data.providers_waiting)}</strong> providers · <strong>{n(data.courses_waiting)}</strong> courses waiting for qualification</p>}
    ...
    <div className="l2po-table-wrap"><table className="l2po-table"><thead><tr><th>Provider</th><th>Courses waiting</th><th>State</th><th>Last submission</th><th/></tr></thead><tbody>
      {rows.map(r=><React.Fragment key={r.provider_id}><tr className={open===r.provider_id?'on':''}>
        ...
        <td><span className={`l2po-state s-${r.state}`}>{LABEL[r.state]||r.state}</span></td>
        <td>{r.last_submission?<small>{r.last_submission.dry_run?'Checked':'Submitted'} {when(r.last_submission.at)}</small>:<small>—</small>}</td>
      ...
      </tr>{open===r.provider_id&&<tr className="l2po-detail"><td colSpan={5}><Onboard row={r} rank={rank} onDone={load}/></td></tr>}</React.Fragment>)}
    </tbody></table></div>}
...
function Onboard({row,rank,onDone}){
