...
describe('CF-241 forward runtime reconciliation contract', () => {
  it('restores the governed lineage subsystem before routing Evidence reads to it', () => {
    const lineageTable = sql.indexOf('create table if not exists pipeline.evidence_lineage_stats')
    const evidencePage = sql.indexOf('create or replace function security.admin_evidence_page')
    ...
    expect(lineageTable).toBeGreaterThanOrEqual(0)
    expect(evidencePage).toBeGreaterThan(lineageTable)
    expect(dispatcher).toBeGreaterThan(evidencePage)
    expect(sql).toContain('create or replace function security.adjust_evidence_lineage_stats')
    expect(sql).toContain('create or replace function security.sync_evidence_lineage_stats')
    expect(sql).toContain('create or replace function security.evidence_row_source_null')
    expect(sql).toContain('create or replace function security.evidence_row_rejected')
    expect(sql).toContain('cf241_evidence_lineage_rebuild')
    expect(sql).toContain('evidence_lineage_stats_sync')
  })

  it('keeps Evidence freshness filters aligned with returned state precedence', () => {
    const nonStale = "lower(coalesce(e.metadata->>'freshness_state',''))<>'stale' and lower(coalesce(e.metadata->>'stale','false'))<>'true'"
    expect(sql).toContain(`v_freshness='expired' and ${nonStale}`)
    expect(sql).toContain(`v_freshness='current' and ${nonStale}`)
    expect(sql).toContain(`v_freshness='no_policy' and ${nonStale}`)
    expect(sql).toContain("'freshness_state',case when x.explicit_stale then 'stale'")
  })

  it('restores the governed evidence page contract', () => {
