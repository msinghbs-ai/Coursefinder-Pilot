...
export default function ComparisonWorkspace({routeParams,navigate,onError}){
 ...
 function switchType(next){setQuery('');setSearchRows([]);setProviderQuery('');setProviderRows([]);setProviderId('');setProviderName('');setStudyLevel('');setYear('');setRankingYear('');commitSelection(next,[])}
  ...
  {!ids.length?<section className="cf-compare-empty"><ArrowLeftRight size={32}/><h3>Select up to six {type}s</h3><p>Search above, or open a {type} detail blade and choose Compare.</p></section>:busy&&!data?<section className="cf-compare-empty"><span className="m-spinner"/><p>Loading governed comparison…</p></section>:<>
   ...
   {(datasets.qs||datasets.the)&&<section className="cf-flow-compare">
    ...
    <div className="cf-flow-cards">{items.map(e=><article key={e.id}><strong>{e.name}</strong>
      {datasets.qs&&(viewMode==='trend'?<RankingTrend label="QS" series={e.ranking_context?.qs_wur}/>:<RankingLine label="QS" series={e.ranking_context?.qs_wur} edition={rankingYear}/>)}
      {datasets.the&&(viewMode==='trend'?<RankingTrend label="THE" series={e.ranking_context?.the_wur}/>:<RankingLine label="THE" series={e.ranking_context?.the_wur} edition={rankingYear}/>)}
    </article>)}</div>
   </section>}
