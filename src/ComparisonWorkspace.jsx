...
export default function ComparisonWorkspace({routeParams,navigate,onError}){
 ...
 const rankingYears=useMemo(()=>[...new Set(items.flatMap(e=>['qs_wur','the_wur'].flatMap(k=>(e.ranking_context?.[k]?.history||[]).map(x=>String(x.edition_year)))))].sort((a,b)=>Number(b)-Number(a)),[items])
 ...
 function switchType(next){setQuery('');setSearchRows([]);setStudyLevel('');setYear('');setRankingYear('');commitSelection(next,[])}
 ...
 function clear(){setQuery('');setSearchRows([]);commitSelection(type,[])}
 return <div className="cf-compare-page">
  ...
  <section className="cf-compare-picker" data-compare-picker={type}>
   ...
   <label className="cf-compare-search"><Search size={16}/><input aria-label={`Search ${type}s to compare`} value={query} onChange={e=>setQuery(e.target.value)} placeholder={`Search ${type}s to compare…`}/>{searchBusy&&<span className="m-spinner tiny"/>}</label>
   {searchRows.length>0&&<div className="cf-compare-results">{searchRows.map(r=><button type="button" key={r.id??r.course_id} disabled={ids.length>=MAX} onClick={()=>add(r)}><span><strong>{r.canonical_name||r.canonical_title||r.name}</strong><small>{type==='course'?[r.provider_name,r.course_code].filter(Boolean).join(' · '):[r.country_code,r.subdivision_name,r.city].filter(Boolean).join(' · ')}</small></span><b>Add +</b></button>)}</div>}
  </section>

  <section className="cf-compare-config">
   <div><strong>2. Choose statistics</strong><span>Select only the datasets you want in this comparison.</span></div>
   <div className="cf-dataset-toggles">
   ...
   <label>3. QILT year<select value={year} onChange={e=>setYear(e.target.value)}><option value="">Latest / all aligned periods</option>{years.map(y=><option key={y} value={y}>{y}</option>)}</select></label>
   {(hasQS||hasTHE)&&<label>Ranking edition<select value={rankingYear} onChange={e=>setRankingYear(e.target.value)}><option value="">Latest available</option>{rankingYears.map(y=><option key={y} value={y}>{y}</option>)}</select></label>}
  </section>
  ...
  {!ids.length?<section className="cf-compare-empty"><ArrowLeftRight size={32}/><h3>Select up to six {type}s</h3><p>Search above, or open a {type} detail blade and choose Compare.</p></section>:busy&&!data?<section className="cf-compare-empty"><span className="m-spinner"/><p>Loading governed comparison…</p></section>:<>
   ...
   {(datasets.qs||datasets.the)&&<section className="cf-flow-compare">
    <div className="cf-flow-title"><div><h3>Institutional world rankings</h3><p>QS and THE remain independent ranking systems and are Provider context for Course comparisons.</p></div><span>Rankings</span></div>
    <div className="cf-flow-cards">{items.map(e=><article key={e.id}><strong>{e.name}</strong>
      {datasets.qs&&<RankingLine label="QS" series={e.ranking_context?.qs_wur} edition={rankingYear}/>}
      {datasets.the&&<RankingLine label="THE" series={e.ranking_context?.the_wur} edition={rankingYear}/>}
    </article>)}</div>
   </section>}
...
.cf-compare-config{background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:12px;display:grid;grid-template-columns:minmax(190px,1fr) minmax(260px,2fr) minmax(200px,1fr);gap:14px;align-items:end}.cf-compare-config>div>strong,.cf-compare-config>div>span{display:block}.cf-compare-config>div>strong{font-size:10px}.cf-compare-config>div>span{font-size:8px;color:#64748b;margin-top:2px}.cf-dataset-toggles{display:flex;gap:6px;flex-wrap:wrap}.cf-dataset-toggles button{border:1px solid #dbe3ee;background:#fff;border-radius:999px;padding:7px 10px;font-size:8.5px;font-weight:850;cursor:pointer}.cf-dataset-toggles button.active{background:#172554;color:#fff;border-color:#172554}.cf-dataset-toggles button:disabled{opacity:.48;cursor:not-allowed}.cf-compare-config label{font-size:8px;color:#64748b;font-weight:800}.cf-compare-config select{display:block;width:100%;margin-top:4px;border:1px solid #dbe3ee;border-radius:7px;padding:7px 8px;background:#fff;font:inherit;font-size:9px}
