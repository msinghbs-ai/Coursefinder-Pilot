  ...
  return <div className="stack">
    ...
    <section className="panel full ingestion-control">
      ...
      <div className="control-steps" style={{marginTop:18}}>
        <ControlStep number="1" title="Validate batch" text={`Fetch and parse ${policy.label} using ${policy.adapter}. Offset ${Number(offset).toLocaleString()}, up to ${batchValue.toLocaleString()} records. No catalogue writes.`} status={runResult?.controlKind === 'dry-run' ? 'done' : 'ready'}><button className="secondary" onClick={()=>execute({apply:false,targetOffset:offset,kind:'dry-run'})} disabled={runBusy}><Play size={15}/>{runBusy ? 'Running…' : 'Validate batch'}</button></ControlStep>
        <ControlStep number="2" title="Apply bounded batch" text="Write only this bounded slice through the country adapter, then rebuild Search Projection and return the next offset." status={runResult?.mode === 'apply' ? 'done' : 'guarded'}><button className="danger-soft" onClick={()=>requestApply(offset,'apply-batch')} disabled={runBusy}><AlertTriangle size={15}/>Apply {country} batch</button></ControlStep>
        <ControlStep number="3" title="Continue" text="Advance to the exact next offset returned by the adapter. This keeps production ingestion deterministic and restartable." status={hasMore ? 'ready' : runResult ? 'done' : 'waiting'}><button className="secondary" onClick={()=>requestApply(nextOffset,'next-batch')} disabled={runBusy || !hasMore}><StepForward size={15}/>Run next batch</button></ControlStep>
        <ControlStep number="4" title="Idempotency check" text="Re-run the same bounded offset. Expected result: zero duplicate identities or depth relationships." status={runResult?.mode === 'apply' ? 'ready' : 'waiting'}><button className="secondary" onClick={()=>requestApply(currentOffset,'idempotency-rerun')} disabled={runBusy || runResult?.mode !== 'apply'}><Repeat2 size={15}/>Re-run current batch</button></ControlStep>
      </div>
    </section>
    ...
    <section className="panel settings-note"><div><strong>Production execution</strong><span>Run one country at a time. Every request carries a deterministic offset and bounded batch size, so failed runs can be restarted without replaying the whole source.</span></div><div><strong>Australia</strong><span>AU uses the consolidated CRICOS ZIP with regulator codes as canonical identity and automatically reconciles matching campuses and course locations for each course slice.</span></div><div><strong>Other countries</strong><span>The same Layer 1 ETL endpoint routes GB, DE, CA, IE, NZ and US through their configured adapters. Their source implementation can be replaced without changing the Settings execution contract.</span></div></section>

    <section className="panel full"><div className="panel-title"><div><span className="kicker">Danger zone</span><h3>Reset Pilot database</h3><p>UAT-only destructive reset. This is not part of normal production ingestion.</p></div><button className="danger-soft" onClick={()=>{setConfirmMode('reset');setConfirmText('')}} disabled={runBusy}><RotateCcw size={15}/>Reset Database</button></div></section>
