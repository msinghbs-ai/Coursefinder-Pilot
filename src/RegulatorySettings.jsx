...
export default function RegulatorySettings({ onError }) {
  ...
  const auRows = rows.filter(r => r.country_code === 'AU')
  const auSources = auRows.filter(r => r.source_id)
  const auHealthy = auSources.filter(r => r.last_success_at && (!r.last_failure_at || new Date(r.last_success_at) >= new Date(r.last_failure_at)))
  const configuredSources = rows.filter(r => r.source_id).length
  const activeCountries = new Set(rows.filter(r => r.source_id && ['pilot','active'].includes(r.catalogue_status)).map(r => r.country_code)).size
  ...
  return <div className="stack">
    <div className="section-head">
      <div><span className="kicker">Platform Admin · Layer 1</span><h2>Regulatory Sources</h2><p>Authoritative regulatory source registry used by Supabase Edge Functions. Source endpoints are configuration, not frontend code.</p></div>
