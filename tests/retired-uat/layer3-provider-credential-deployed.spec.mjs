...
import { openGovernanceProvider, openLayer3 } from './support/navigation.mjs'
...
test.describe('CourseFinder Layer 3 provider credential control @deployed',()=>{
  test.beforeAll(async()=>{
    ...
    await writeRunEnvironment({suite:'deployed-layer3-provider-credential-v2',change_control:'CF-CHG-20260825-038'})
  })
  ...
  test('credential UI obeys the Platform Admin boundary and remains write-only when authorised',async({page},testInfo)=>{
    ...
    try{
      ...
      await expect(page.locator('#governed-runtime-marker')).toHaveCount(0);await expect(page.locator('.m-release-pill')).toContainText(/v2\./)
      ...
      const dialog=await openGovernanceProvider(page)
