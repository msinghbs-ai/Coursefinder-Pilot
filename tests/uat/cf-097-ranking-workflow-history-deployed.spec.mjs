...
test.describe('CF-097/226 ranking workflow, history and datasets @deployed',()=>{
 ...
 test('Statistics exposes accepted QS and THE datasets and opens imported observations',async({page},testInfo)=>{const runtime=observeRuntime(page);try{
  await loginAsUatUser(page)
  ...
  await expect(qs.getByRole('button',{name:'Open dataset'})).toBeVisible()
  ...
  const year=viewer.locator('[data-year]')
