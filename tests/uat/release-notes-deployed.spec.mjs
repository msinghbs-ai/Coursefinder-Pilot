...
test.describe('CourseFinder PIM Admin v2.15.18 release notes @deployed',()=>{
 ...
 test('top-right version opens maintained release history and closes accessibly',async({page},testInfo)=>{const runtime=observeRuntime(page);try{
  ...
  await expect(version).toContainText('v2.15.18')
  ...
  await expect(dialog.locator('[data-release-version="2.15.18"]')).toContainText('Platform maturity Administration workspace')
  ...
  await milestoneScreenshot(page,testInfo,'pim-v2-15-18-release-notes')
