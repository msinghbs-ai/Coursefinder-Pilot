...
test.describe('CourseFinder PIM Admin v2.15.19 release notes @deployed',()=>{
 ...
 test('top-right version opens maintained release history and closes accessibly',async({page},testInfo)=>{const runtime=observeRuntime(page);try{
  ...
  await expect(version).toContainText('v2.15.19')
  ...
  await expect(dialog.locator('[data-release-version="2.15.19"]')).toContainText('Evidence lineage reconciliation and contact claim hardening')
  ...
  await milestoneScreenshot(page,testInfo,'pim-v2-15-19-release-notes')
