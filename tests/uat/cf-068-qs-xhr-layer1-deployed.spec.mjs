...
test.describe('CF-068 QS direct XHR Layer 1 acquisition @deployed',()=>{
 ...
 test('QS 2026 validates 1501 publisher rows from retained direct XHR JSON',async({page},testInfo)=>{test.setTimeout(180000);const runtime=observeRuntime(page);try{
  ...
  const preview=result.body?.validation?.discovered?.reconciliation_preview;expect(preview?.country_code).toBe('AU');expect(preview?.country_rows).toBeGreaterThan(30);expect(preview?.exact_unique).toBeGreaterThan(20);expect(preview?.exact_ambiguous).toBe(0);expect(preview?.unmatched).toBeGreaterThanOrEqual(0)
