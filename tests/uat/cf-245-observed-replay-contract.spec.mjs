...
test.describe('CF-245 stored Evidence replay contract',()=>{
 test('reuses only qualified deterministic facts and isolates tuition for Layer 3',async()=>{
  ...
  const sql=base+'\n'+fix
  for(const text of [
   'svc_cf245_replay_observed_coursefacts',
   "qualification_status in ('qualified','bounded')",
   "apply_admitted",
   "identity_match",
   "regulatory_code_seen",
   "layer4_entity_or_parent_blocked('course'",
   "'intake'=any(admitted_domains)",
   "'english_requirement'=any(admitted_domains)",
   "between 1 and 9",
   "between 10 and 90",
   "between 1 and 120",
   "deterministic_observed_replay",
   'cf245_layer3_fee_validation_backlog_v1',
   'requires_layer3_fee_validation',
   'benchmark_required_before_automatic_interpretation',
   'CF-CHG-20260915-245',
