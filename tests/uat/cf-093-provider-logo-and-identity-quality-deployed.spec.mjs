...
test.describe('CF-093 Provider logo UX and identity quality @targeted',()=>{
  test.beforeAll(async()=>{await writeRunEnvironment({suite:'cf-093-provider-logo-identity-quality-v1',change_control:'CF-CHG-20260904-093'})})
  ...
  test('deployed Provider list renders logo and detail name uses theme text',async({page},testInfo)=>{
    ...
    try{
      ...
      await milestoneScreenshot(page,testInfo,'cf-093-provider-logo-list-and-detail')
  ...
  test('deployed Provider location does not repeat institution name',async({page},testInfo)=>{
    ...
    try{
      ...
      await milestoneScreenshot(page,testInfo,'cf-093-provider-location-quality')
    }finally{await finish(testInfo,runtime)}
  })
})
