...
test.describe('CourseFinder deployed Course Detail PIM v2.15.x standardised operator UX @deployed',()=>{
  ...
  test('Course decision-card order persists for the signed-in user and can be restored',async({page},testInfo)=>{
    ...
    try{
      ...
      let headings=drawer.locator('.cf-reorder-wrap section h3');await expect(headings.first()).toHaveText('Locations');await page.reload();await expect(drawer).toBeVisible({timeout:45_000});headings=drawer.locator('.cf-reorder-wrap section h3');await expect(headings.first()).toHaveText('Locations');await clearCourseLayoutPreference(page);await page.reload();await expect(drawer).toBeVisible({timeout:45_000});await expect(drawer.locator('.cf-reorder-wrap section h3').first()).toHaveText('Fees & entry requirements')
