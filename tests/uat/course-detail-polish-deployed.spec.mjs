...
test.describe('CourseFinder deployed Course Detail PIM v2.15.x standardised operator UX @deployed',()=>{
  test.beforeAll(async()=>{
    ...
    await writeRunEnvironment({suite:'deployed-course-detail-v2.15-standardised-layout'})
  })
  ...
  test('Federation Bachelor of Arts renders standardised decision layout and Evidence remains responsive',async({page},testInfo)=>{
    ...
    try{
      ...
      await expect(page.locator('#governed-runtime-marker')).toHaveCount(0);await expect(page.locator('.m-release-pill')).toContainText('v2.15.')
  ...
  test('Course decision-card order persists for the signed-in user and can be restored',async({page},testInfo)=>{
    const runtime=observeRuntime(page)
    try{
      await loginAsUatUser(page);await clearCourseLayoutPreference(page);await page.goto(`${process.env.UAT_BASE_URL}/#courses?id=${ARTS_ID}`)
      const drawer=page.locator('aside.m-drawer');await expect(drawer).toBeVisible({timeout:45_000});await expect(drawer.locator('.cf-reorder-wrap section h3').first()).toHaveText('Fees & entry requirements')
      await drawer.getByRole('button',{name:'Arrange sections',exact:true}).click();await drawer.getByRole('button',{name:'Move Locations up',exact:true}).click();await drawer.getByRole('button',{name:'Done arranging',exact:true}).click()
      let headings=drawer.locator('.cf-reorder-wrap section h3');await expect(headings.nth(0)).toHaveText('Fees & entry requirements');await expect(headings.nth(1)).toHaveText('Locations');await page.reload();await expect(drawer).toBeVisible({timeout:45_000});headings=drawer.locator('.cf-reorder-wrap section h3');await expect(headings.nth(0)).toHaveText('Fees & entry requirements');await expect(headings.nth(1)).toHaveText('Locations');await clearCourseLayoutPreference(page);await page.reload();await expect(drawer).toBeVisible({timeout:45_000});await expect(drawer.locator('.cf-reorder-wrap section h3').first()).toHaveText('Fees & entry requirements')
