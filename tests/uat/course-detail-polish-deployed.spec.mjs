...
test.describe('CourseFinder deployed Course Detail PIM v2.15.1 recovery + field-state acceptance @deployed',()=>{
  test.beforeAll(async()=>{
    ...
    await writeRunEnvironment({suite:'deployed-course-detail-v2.15.1-recovery-field-state'})
  })

  test('Course drawer renders safely and every governed attribute remains visible',async({page},testInfo)=>{
    ...
    try{
      ...
      await milestoneScreenshot(page,testInfo,'course-drawer-v2-15-1-render-safe')
  ...
  test('Science Honours distinguishes not-attempted L2, failed L2 and direct L4 fields',async({page},testInfo)=>{
    ...
    try{
      ...
      await milestoneScreenshot(page,testInfo,'science-honours-v2-15-1-layer-state-matrix')
    }finally{await finish(testInfo,runtime)}
  })
})
