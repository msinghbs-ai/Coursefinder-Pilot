...
assert.match(
  benchmarkSource,
  /call\(\s*profile,\s*key,\s*`governed-\$\{c\.provider_cricos\}-\$\{c\.course_cricos\}`,\s*evidence,\s*c\.candidate_payload,\s*candidateContext,?\s*\)/,
  "the provider-loop call() must be invoked with the explicit candidateContext",
);
assert.match(
  benchmarkSource,
  /call\(\s*profile,\s*key,\s*c\.case,\s*c\.text,\s*c\.candidate,\s*candidateContext,?\s*\)/,
