...
begin
  if p_function is null or p_function not in ('layer1-ca-on-college-programs','layer1-ca-algonquin-catalogue','layer1-ca-conestoga-catalogue') then
...
as $$
begin
  if new.function_name not in ('layer1-ca-on-college-programs','layer1-ca-algonquin-catalogue','layer1-ca-conestoga-catalogue') then
    new.status:='failed'; new.error_text:='function not allowlisted'; return new;
  end if;
  begin
  ...
  exception when others then
    new.status:='failed'; new.error_text:=sqlerrm;
  end;
  return new;
end $$;