...
begin
  ...
  if v_policy_gaps>0 then
    v_result:=v_result||jsonb_build_object('executable',false,'execution_block_reason','One or more fully queueable Layer 2 profiles in this scope do not have an execution policy. Configure the execution policy before dispatch.');
    return v_result||jsonb_build_object('preview_token',null,'preview_expires_at',null);
  end if;
...
declare
  ...
  v_preview pipeline.jobs%rowtype; v_prior pipeline.jobs%rowtype; v_live_preview jsonb; v_result jsonb; v_scope_key text; v_work integer:=0; v_live_work integer:=0; v_invalid_profiles integer:=0; v_policy_gaps integer:=0;
begin
  if v_actor is null or security.current_role_rank() < 4 then raise exception 'pipeline operator role required' using errcode='42501'; end if;
