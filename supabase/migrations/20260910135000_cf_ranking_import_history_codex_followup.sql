...
-- Post-merge correction for PR #59 Codex findings.
      ...
      'items',coalesce((
        ...
        from (
          select mi.id,s.code system_code,mi.edition_year,mi.publisher_name,mi.source_url,mi.methodology_url,
            ...
            coalesce((
              select jsonb_agg(distinct scope_item.value)
              ...
              where mi_scope.system_id=mi.system_id
                and mi_scope.edition_year=mi.edition_year
            ),'[]'::jsonb) detected_scope,
            (select count(*)
             from ranking.manual_imports mi2
             where mi2.system_id=mi.system_id
               and mi2.edition_year=mi.edition_year)::integer logical_revision_count,
