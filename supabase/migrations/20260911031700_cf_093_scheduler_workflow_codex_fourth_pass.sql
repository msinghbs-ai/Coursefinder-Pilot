...
begin
  ...
  -- Preview ownership remains actor-bound.
  ...
  select count(distinct sc.profile_id)::integer
  ...
  -- Exact-scope paid acquisition dedupe is intentionally operator-independent.
  -- The actor boundary applies to the preview token above, not to recent successful
  -- dispatch receipts for the same governed target.
  select * into v_prior
  from pipeline.jobs j
  where j.id<>p_preview_token
    and j.job_type='scheduler_workflow_preview'
  ...
  if found then
    ...
    set payload=payload||jsonb_build_object(
      'consumed_at',now(),
      'dispatch_result',v_prior.payload->'dispatch_result',
      'deduplicated_against_preview',v_prior.id,
      'governance_reason',trim(p_reason)
    )
  ...
  -- The start call and this validation are in the same transaction. If the scope
  -- became empty between preview and start, raising here rolls back all start-side
  -- effects and leaves the preview token unconsumed for a fresh preview.
  if jsonb_typeof(v_result->'profiles')<>'array'
     or jsonb_array_length(coalesce(v_result->'profiles','[]'::jsonb))=0 then
