-- CF-CHG-20260915-247
-- Reconcile the one-time tuition benchmark Edge worker with the governed Pilot nonce allowlist.
create or replace function pipeline.svc_pilot_submit_nonce(p_function text,p_body jsonb default '{}'::jsonb)
returns bigint language plpgsql security definer set search_path='pipeline','net','public','extensions' as $$
...
begin
 if p_function not in(
  ...
  'layer3-cf245-tuition-benchmark'
 ) then raise exception 'one-time Edge function is not allowlisted';end if;
 ...
 select net.http_post(url:=rtrim(v_base,'/')||'/'||p_function,headers:=jsonb_build_object('content-type','application/json','x-cf-run-nonce',v_nonce::text),body:=coalesce(p_body,'{}'::jsonb),timeout_milliseconds:=120000) into v_id;
 return v_id;
end $$;
