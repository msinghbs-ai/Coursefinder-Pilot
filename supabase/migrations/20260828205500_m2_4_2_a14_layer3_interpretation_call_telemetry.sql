alter table pipeline.layer3_interpretations
  ...
  add column if not exists call_latency_ms integer;

alter table pipeline.layer3_interpretations
  drop constraint if exists layer3_interpretations_external_call_count_check,
  add constraint layer3_interpretations_external_call_count_check check (external_call_count is null or external_call_count >= 0),
  drop constraint if exists layer3_interpretations_call_latency_ms_check,
  add constraint layer3_interpretations_call_latency_ms_check check (call_latency_ms is null or call_latency_ms >= 0);

create or replace function public.layer3_complete_interpretation_service(
  p_interpretation_id uuid,
  p_raw_result jsonb,
  p_candidate_value jsonb,
  p_confidence numeric,
  p_rationale text,
  p_evidence_quotes jsonb,
...
begin
  ...
  update pipeline.layer3_interpretations set
    ...
    candidate_value=p_candidate_value,
    ...
    aggregator_response_model=p_response_model,
    ...
    interpretation_expires_at=p_expiry
  where id=p_interpretation_id;

  if p_valid and p_candidate_value is not null and p_candidate_value<>'null'::jsonb then
    ...
    values(
      ...
      jsonb_build_object(
        ...
        'validator_result',p_validator_result,'response_model',p_response_model,
        'external_call_count',p_external_call_count,'call_latency_ms',p_call_latency_ms,
        'input_tokens',p_input_tokens,'output_tokens',p_output_tokens,'estimated_cost_usd',p_estimated_cost_usd
      ),
      'validated Layer 3 candidate requires human decision'
    ) returning id into v_review;
  end if;
  return jsonb_build_object(
    'ok',true,'interpretation_id',p_interpretation_id,'validated',p_valid,'review_item_id',v_review,
    'external_call_count',p_external_call_count,'call_latency_ms',p_call_latency_ms
  );
end $$;
...
create or replace function public.layer3_fail_interpretation_service(
  p_interpretation_id uuid,
  p_error text,
  p_external_call_count integer,
  p_call_latency_ms integer
...
revoke all on function public.layer3_fail_interpretation_service(uuid,text,integer,integer) from public, anon, authenticated;
grant execute on function public.layer3_fail_interpretation_service(uuid,text,integer,integer) to service_role;
