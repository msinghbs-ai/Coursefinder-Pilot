...
begin
  ...
  return jsonb_build_object(
    ...
    'provider_stats_summary',(select jsonb_build_object(
      'providers',count(*),
      'canonical_total',coalesce(sum(canonical_total),0),
      'published_total',coalesce(sum(published_total),0),
      'unpublished_total',coalesce(sum(unpublished_total),0),
      'first_party_verified_total',coalesce(sum(first_party_verified_total),0),
      'layer4_linked_total',coalesce(sum(layer4_linked_total),0),
      'canonical_linked_total',coalesce(sum(canonical_linked_total),0),
      'publication_decision_linked_total',coalesce(sum(publication_decision_linked_total),0),
      'benchmarked_providers',count(*) filter(where landscape_benchmark_total is not null),
      'last_trace_update',max(trace_last_updated_at)
    ) from pipeline.scholarship_provider_stats),
