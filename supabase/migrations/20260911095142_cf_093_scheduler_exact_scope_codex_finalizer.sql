...
as $function$
  with scoped as materialized (
    ...
    select count(*) filter (where source_url is not null)::integer queueable_count,
           ...
           coalesce(md5(string_agg(profile_id::text||':'||course_id::text||':'||coalesce(source_url,'<discover>'),'|' order by profile_id,course_id,coalesce(source_url,'<discover>'))),md5('')) scope_fingerprint
    from scoped
  )
  ...
  from (
    ...
    where not exists (
      ...
      where r.profile_id=sc.profile_id
        ...
        and (lower(coalesce(ap.auth_scheme,'none'))='none' or ap.vault_secret_id is not null)
    )
  ) gaps
$function$;
...
as $function$
  ...
  where target_url is null
     or target_url !~* '^https://[A-Za-z0-9._~-]+(?::[0-9]+)?(?:/|\?|#|$)'
     or target_url ~ '[[:space:]]'
$function$;
  ...
  if v_kind='country' then
    ...
    with q as (
      ...
      where upper(co.iso_alpha2::text)=v_country
        ...
        and exists (select 1 from public.layer2_scope_courses(v_country,'state',sd.id) sc)
    ), n as (select count(*) total from q), page as (select * from q order by lower(label),value limit v_limit offset v_offset)
    select coalesce((select jsonb_agg(jsonb_build_object('value',value,'label',label,'meta',meta) order by lower(label),value) from page),'[]'::jsonb),coalesce((select total from n),0)
    ...
    with scoped as materialized (
      select distinct sc.provider_id,sc.provider_name
      from public.layer2_scope_courses(v_country,case when p_state_id is null then 'country' else 'state' end,p_state_id) sc
    ), q as (
      select provider_id::text value,provider_name label,'Qualified for Layer 2'::text meta
      from scoped
      where v_query is null or lower(provider_name) like '%'||v_query||'%'
    ), n as (select count(*) total from q), page as (select * from q order by lower(label),value limit v_limit offset v_offset)
    select coalesce((select jsonb_agg(jsonb_build_object('value',value,'label',label,'meta',meta) order by lower(label),value) from page),'[]'::jsonb),coalesce((select total from n),0)
    into v_items,v_total;
    return jsonb_build_object('items',v_items,'total',v_total,'limit',v_limit,'offset',v_offset,'has_more',(v_offset+jsonb_array_length(v_items))<v_total,'scope_source','layer2_executable_provider_scope');
  end if;
...
declare
  ...
  v_invalid_profiles integer:=0; v_policy_gaps integer:=0; v_oversized_profiles integer:=0; v_route_gaps integer:=0; v_discovery_config_gaps integer:=0;
...
begin
  ...
  v_result:=coalesce(public.layer2_operator_scope_service(v_actor,'preview',upper(p_country_code),v_scope,p_scope_id),'{}'::jsonb)
    ||jsonb_build_object('queueable_count',coalesce((v_snapshot->>'queueable_count')::integer,0),'needs_discovery_count',coalesce((v_snapshot->>'needs_discovery_count')::integer,0),'scoped_course_count',coalesce((v_snapshot->>'scoped_course_count')::integer,0),'scope_fingerprint',v_snapshot->>'scope_fingerprint');
  ...
  if v_route_gaps>0 then return v_result||jsonb_build_object('executable',false,'execution_block_reason','One or more Layer 2 profiles in this scope have no runtime-usable acquisition route. Restore an enabled non-Parsebot route with required credential before dispatch.','preview_token',null,'preview_expires_at',null); end if;
  if v_discovery_config_gaps>0 then return v_result||jsonb_build_object('executable',false,'execution_block_reason','One or more profiles requiring discovery have no worker-valid HTTPS discovery target. Correct the governed profile configuration before dispatch.','preview_token',null,'preview_expires_at',null); end if;
  if cardinality(v_profile_ids)=0 or coalesce((v_snapshot->>'queueable_count')::integer,0)+coalesce((v_snapshot->>'needs_discovery_count')::integer,0)<=0 then return v_result||jsonb_build_object('executable',false,'execution_block_reason','No executable Layer 2 work is available for this governed scope. Qualify a Layer 2 Course Facts profile or choose a scope with queueable/discovery work.','preview_token',null,'preview_expires_at',null); end if;
...
declare
  ...
  v_preview pipeline.jobs%rowtype; v_prior pipeline.jobs%rowtype; v_live_snapshot jsonb; v_result jsonb; v_scope_key text; v_invalid_profiles integer:=0; v_policy_gaps integer:=0; v_oversized_profiles integer:=0; v_route_gaps integer:=0; v_discovery_config_gaps integer:=0;
...
begin
  ...
  v_discovery_config_gaps:=security.scheduler_workflow_discovery_config_gap_count_v1(upper(p_country_code),v_scope,p_scope_id); if v_discovery_config_gaps>0 then raise exception 'Layer 2 discovery configuration changed after preview; correct the worker-valid HTTPS target and preview again' using errcode='22023'; end if;
...
revoke all on function security.scheduler_workflow_route_gap_count_v1(text,text,uuid) from public,anon,authenticated;
revoke all on function security.scheduler_workflow_discovery_config_gap_count_v1(text,text,uuid) from public,anon,authenticated;
revoke all on function security.scheduler_workflow_scope_options_v1_browser_bridge(text,text,uuid,text,integer,integer) from public,anon;
