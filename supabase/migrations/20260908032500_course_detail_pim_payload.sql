  ...
  select case when c.id is null then null else jsonb_build_object(
    ...
    'pim_attribute_values', coalesce((
      select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
        ...
        'option_labels', coalesce((select jsonb_object_agg(ao.code,ao.label order by ao.display_order nulls last,ao.code) from pim.attribute_options ao where ao.attribute_id=ad.id and coalesce(ao.status,'active')='active'),'{}'::jsonb)
