-- CF-CHG-20260915-247
-- Preserve the deterministic Layer 2 fee candidate at the L2 -> L3 boundary.
-- Layer 3 validates this bounded candidate against retained Evidence; it does not become an unrestricted fee extractor.
begin;

alter table pipeline.layer3_work_items
  add column if not exists candidate_context jsonb;

comment on column pipeline.layer3_work_items.candidate_context is
  'Immutable Layer 2 candidate context supplied to Layer 3 validation; Evidence remains authoritative and AI cannot expand beyond these candidates.';
...
begin
  ...
  with profile as (
    ...
    select i.id as layer2_run_item_id,
           ...
           jsonb_build_object(
             'provider_current_tuition',sr.parsed_payload->'provider_current_tuition',
             'fee_candidates',coalesce(sr.parsed_payload->'fee_candidates','[]'::jsonb),
             'fee_ambiguous',coalesce((sr.parsed_payload->>'fee_ambiguous')::boolean,false),
             'identity_match',coalesce((sr.parsed_payload->>'identity_match')::boolean,false),
             'expected_course_code',sr.parsed_payload->'expected_course_code',
             'extraction_worker',sr.parsed_payload->'extraction_worker',
    ...
    join lateral (
      select r.source_record_id,r.parsed_payload
      from pipeline.course_fact_source_records r
      where r.evidence_id=a.evidence_id
        and r.parsed_payload->>'course_id'=i.entity_id::text
    ...
    where i.status='layer3_required'
      ...
      and (
        sr.parsed_payload ? 'provider_current_tuition'
        or jsonb_array_length(coalesce(sr.parsed_payload->'fee_candidates','[]'::jsonb))>0
      )
      and not exists (
    ...
    select c.layer2_run_item_id,c.evidence_id,lower(i.entity_type),i.entity_id,
           'provider_current_tuition_validation',c.profile_id,c.reason,c.candidate_context
  ...
  select jsonb_build_object(
    ...
    'reason',case when exists(select 1 from profile) then 'eligible_candidate_validation_handoffs_enqueued' else 'no_benchmark_passed_executable_profile' end
