-- M2.4.2 — Advanced Layer 2 configuration must retain visibility of governed paused/disabled profiles.
-- Execution eligibility remains enforced by runtime context/scope services; this is read-only operational visibility.
...
stable
security definer
set search_path to 'pg_catalog','security','pipeline','ref','catalogue','public'
as $$
...
begin
  ...
  if p_operation='layer2_profiles' then
    with job_counts as (
      select source_id,count(*)::bigint job_count from pipeline.jobs where source_id is not null group by source_id
    ), evidence_counts as (
      select source_id,count(*)::bigint evidence_count from pipeline.evidence_artifacts where source_id is not null group by source_id
    ), version_counts as (
      select profile_id,count(*)::bigint version_count from pipeline.layer2_source_profile_versions group by profile_id
    ), rows as (
      select jsonb_build_object(
        ...
        'job_count',coalesce(jc.job_count,0),'evidence_count',coalesce(ec.evidence_count,0),'version_count',coalesce(vc.version_count,0),
        'affected_provider_id',s.provider_id,'affected_provider_name',cp.canonical_name,
        'health',case when not p.enabled then 'disabled' when p.paused then 'paused' when v.validation_status<>'valid' then 'blocked'
      ...
      left join job_counts jc on jc.source_id=p.source_id
      left join evidence_counts ec on ec.source_id=p.source_id
      left join version_counts vc on vc.profile_id=p.id
      where p.domain in ('course_facts','scholarship')
        and p.target_entity_type in ('course_fact','scholarship')
    )
    select coalesce(jsonb_agg(row_json order by lower(row_json->>'source_label')),'[]'::jsonb) into v_result from rows;
    return v_result;
  end if;
  ...
  if p_operation='layer2_profile_detail' then
    ...
    left join pipeline.layer2_source_profile_versions v on v.id=p.current_version_id
    where p.id=v_id
      and p.domain in ('course_facts','scholarship')
      and p.target_entity_type in ('course_fact','scholarship');
...
revoke all on function security.admin_layer2_config_read(text,jsonb) from public,anon;
grant execute on function security.admin_layer2_config_read(text,jsonb) to authenticated,service_role;
