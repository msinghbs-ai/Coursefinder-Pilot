-- CF-CHG-20260910-093
-- Forward-only performance correction for deployed dispatcher tuning read.
-- Bounds provider telemetry to 24h and adds active-window throughput that is not distorted by prior stalls.

begin;

create index if not exists layer2_provider_attempts_profile_provider_created_idx
  on pipeline.layer2_provider_attempts(profile_version_id,acquisition_provider_id,created_at desc);
...
begin
  ...
  select pv.id into v_profile_version_id
  from pipeline.layer2_source_profile_versions pv
  where pv.profile_id=v_profile_id and pv.validation_status='valid'
  ...
  with recent_batches as (
    ...
    select b.id,
      ...
      count(*) filter(where i.completed_at>=now()-interval '60 minutes') recent_completed_60m,
      min(i.completed_at) filter(where i.completed_at>=now()-interval '60 minutes') recent_first_completed_at,
      max(i.completed_at) filter(where i.completed_at>=now()-interval '60 minutes') recent_last_completed_at
    ...
    select jsonb_build_object(
      ...
      'wall_throughput_items_per_min',case when b.started_at is not null and b.processed_count>0 and coalesce(b.completed_at,now())>b.started_at then round((b.processed_count::numeric/nullif(extract(epoch from (coalesce(b.completed_at,now())-b.started_at)),0))*60,2) end,
      'recent_completed_60m',m.recent_completed_60m,
      'active_window_items_per_min',case when m.recent_completed_60m>=2 and m.recent_last_completed_at>m.recent_first_completed_at then round((m.recent_completed_60m::numeric/nullif(extract(epoch from (m.recent_last_completed_at-m.recent_first_completed_at)),0))*60,2) end,
    ...
    select jsonb_build_object(
      ...
      'attempts_24h',count(pa.*),
      'success_24h',count(pa.*) filter(where pa.status='succeeded'),
      'failed_24h',count(pa.*) filter(where pa.status='failed'),
      'http_429_24h',count(pa.*) filter(where pa.response_http_status=429),
      'avg_attempt_ms_24h',round(avg(extract(epoch from (pa.completed_at-pa.started_at))*1000) filter(where pa.completed_at is not null and pa.started_at is not null)::numeric,1)
    ) row_json,r.priority
    ...
    left join pipeline.layer2_provider_attempts pa on pa.acquisition_provider_id=ap.id
      and pa.profile_version_id=v_profile_version_id
