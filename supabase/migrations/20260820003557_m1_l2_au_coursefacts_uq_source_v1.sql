...
values('au_uq_course_pages','The University of Queensland official program pages','provider_course_pages','https://study.uq.edu.au','active',jsonb_build_object('country_code','AU','provider_cricos','00025B','identity_mapping','exact_cricos_course_code'))
...
select 'provider_course_page',sys.id,p.id,p.country_id,'https://study.uq.edu.au/study-options/programs','The University of Queensland official program pages',95,'active',jsonb_build_object('facts',jsonb_build_array('course_link','international_fee','intake','english_requirement'),'course_identity','exact_cricos_course_code','provider_cricos','00025B')
from integration.systems sys
join catalogue.providers p on p.stable_key='provider:cricos:00025b'
where sys.code='au_uq_course_pages'
and not exists(select 1 from pipeline.sources s where s.system_id=sys.id and s.provider_id=p.id and s.status='active');
...
select p.country_id,s.id,'au_uq_official_program_pages','The University of Queensland','provider_owned_program_pages','00025B','exact_provider_cricos_plus_course_cricos','fresh_html_snapshot_sha256','bounded',array['official_course_url','international_fee','intake','english_requirement']::text[],'Second AU Provider source class; Search remains separately blocked.',jsonb_build_object('gate','M1-L2-AU-COURSE-FACTS','worker','coursefacts-au-uq','apply_admitted',true,'search_admitted',false,'identity_authority',false,'source_class_uat','pending')
from pipeline.sources s
join catalogue.providers p on p.id=s.provider_id
join integration.systems sys on sys.id=s.system_id and sys.code='au_uq_course_pages'
...
begin
  if p_function not in (
    ...
    'prisms-au-etl',
    'layer1-au-depth',
    'layer1-au-completeness',
    'coursefacts-au-rmit',
    'coursefacts-au-uq',
    'layer1-au-cricos-facts'
  ) then
