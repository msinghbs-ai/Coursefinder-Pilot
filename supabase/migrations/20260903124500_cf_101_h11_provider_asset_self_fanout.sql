...
begin
 ...
 insert into pipeline.layer2_shared_fetches(
   ...
   jsonb_build_object('registered_by','layer2-acquire-v2','canonical_mutation_authorised',false,'change_control_ref','CF-CHG-20260903-101')
 )
 ...
 select v_id,p.id,
   case when p.target_entity_type='provider_asset' then 'provider_asset'
        when p.domain='scholarship' then 'scholarship_discovery'
        else 'extract' end,
   jsonb_build_object('source_profile_id',p_profile_id,'source_id',v_source,'shared_evidence_id',p_evidence_id,'change_control_ref','CF-CHG-20260903-101')
 from pipeline.layer2_source_profiles p
 where p.source_id=v_source
   and p.enabled and not p.paused
   and (p.id<>p_profile_id or p.domain='provider_asset')
