...
-- Forward-only hardening before runtime apply: never infer binding presence from a stale
-- PL/pgSQL FOUND value. Legacy service-only discovery remains unchanged when no exact
-- scheduler binding is present.
