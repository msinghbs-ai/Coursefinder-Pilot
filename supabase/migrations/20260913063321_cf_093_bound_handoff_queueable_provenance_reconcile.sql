-- CF-CHG-20260910-093
-- Forward-only correction: exact Preview provenance applies to discovery-subset outcomes,
-- while queueable URLs already covered by the binding queueable fingerprint remain usable.
-- Applied predecessor migrations remain immutable.
...
begin
  ...
  with chosen as (
    select c.id,
      case
        when v_bound and c.id=any(v_binding.discovery_course_ids) then coalesce((
