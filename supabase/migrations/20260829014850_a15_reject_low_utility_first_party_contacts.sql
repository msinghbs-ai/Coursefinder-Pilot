-- A15: preserve but reject low-utility first-party observations with neither a usable
-- institutional contact method nor a named territory assignment.
update pipeline.provider_contact_observations
set verification_state='rejected',
    is_current=false,
    valid_to=coalesce(valid_to,now()),
    metadata=metadata||jsonb_build_object(
      'a15_quality_disposition','rejected_low_utility_contact',
      'a15_quality_review_at',now(),
      'superseded_by_worker','provider-contact-discover-scheduled-v1.2.2'
    ),
    updated_at=now()
where source_class='first_party'
  ...
  and not (
    ...
    and nullif(trim(coalesce(territory_text,'')),'') is not null
  );
...
select o.provider_id,o.id,'contact_removed',o.source_class,
       ...
       jsonb_build_object('reason','a15_low_utility_contact','worker_version','provider-contact-discover-scheduled-v1.2.2','a15_quality_probe',true)
from pipeline.provider_contact_observations o
where o.metadata->>'a15_quality_disposition'='rejected_low_utility_contact'
  and not exists (
    select 1 from pipeline.provider_contact_watch_events e
    where e.observation_id=o.id and e.metadata->>'reason'='a15_low_utility_contact'
  );
