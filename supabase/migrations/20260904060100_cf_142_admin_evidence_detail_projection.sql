-- CF-142 — project acquisition provenance through the existing guarded Evidence-detail contract.
create or replace function security.admin_evidence_detail(p_evidence_id uuid)
returns jsonb
language plpgsql
stable security definer
set search_path to 'pg_catalog','security','pipeline','workflow','ref','auth','storage'
as $function$
declare
  v_rank integer:=0;
  v_result jsonb;
...
begin
  ...
  select jsonb_build_object(
    ...
    'related_visual',security.admin_evidence_related_visual(e.id),
