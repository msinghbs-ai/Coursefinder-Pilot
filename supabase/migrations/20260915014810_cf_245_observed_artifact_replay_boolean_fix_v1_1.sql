-- CF-CHG-20260915-245
-- Forward-only fix: sparse/empty extracted English payloads must evaluate safety predicates to false, never SQL NULL.
...
begin
  ...
  select id,source_id,evidence_id,course_id,provider_cricos,course_cricos,source_url,parsed_payload,search_admitted,
    coalesce(('intake'=any(admitted_domains)) and jsonb_typeof(coalesce(parsed_payload->'intakes','[]'::jsonb))='array'
      and exists(select 1 from jsonb_array_elements(coalesce(parsed_payload->'intakes','[]'::jsonb)) x(v)
        where jsonb_typeof(x.v)='string' and lower(trim(both '"' from x.v::text)) in ('january','february','march','april','may','june','july','august','september','october','november','december')),false) as intake_safe,
    coalesce(('english_requirement'=any(admitted_domains)) and (
      ...
      or nullif(parsed_payload->'english_requirements'->>'toefl_overall','')::numeric between 1 and 120),false) as english_safe,
    coalesce(not intake_exists,false),coalesce(not english_exists,false),
    coalesce(('international_fee'=any(admitted_domains)) and coalesce((parsed_payload->>'fee_safe')::boolean,false)
      and nullif(parsed_payload->'provider_current_tuition'->>'amount','')::numeric>0
      and coalesce(parsed_payload->'provider_current_tuition'->>'basis','')='annual_or_indicative_requires_validation',false) as fee_requires_layer3
  from resolved
