...
-- CF-CHG-20260910-093
-- Additive runtime reconciliation after the earlier CF-093 scheduler list contract was already applied to Pilot.
-- Keeps repository migration history aligned with the deployed Pilot function semantics.
        ...
        select p.id,p.country_code,p.layer,p.source_id,p.source_profile_id,p.entity_type,p.entity_id,
               ...
               coalesce(
                 ...
                 case
                   ...
                   when exists(
                     ...
                     where au.id=p.created_by
                       and au.deleted_at is null
                       and (au.banned_until is null or au.banned_until <= now())
                   ) then security.scheduler_actor_display(p.created_by)
                   else 'Former user'
                 end
               ...
               coalesce(
                 ...
                 case
                   ...
                   when exists(
                     select 1 from auth.users au
                     where au.id=p.owner_user_id
                       and au.deleted_at is null
                       and (au.banned_until is null or au.banned_until <= now())
                   ) then security.scheduler_actor_display(p.owner_user_id)
                   else 'Former user'
                 end
               ) as owner_display,
               case
...
create or replace function public.scheduler_policies_list_v1(
  p_limit integer default 50,
  p_offset integer default 0,
  p_query text default ''
) returns jsonb
...
set search_path=''
as $function$
  select security.scheduler_policies_list_v1_browser_bridge(p_limit,p_offset,p_query)
$function$;

revoke all on function security.scheduler_policies_list_v1_browser_bridge(integer,integer,text) from public,anon;
grant execute on function security.scheduler_policies_list_v1_browser_bridge(integer,integer,text) to authenticated,service_role;
revoke all on function public.scheduler_policies_list_v1(integer,integer,text) from public,anon;
grant execute on function public.scheduler_policies_list_v1(integer,integer,text) to authenticated,service_role;
