-- CF-CHG-20260915-247, Track B (consolidate execution without big-bang
-- refactor). Retires the tuition-specific admin panel added earlier this
-- programme unit: it duplicated functionality that already exists
-- generically in security.layer3_model_profiles_admin_impl (used by the
-- real Layer 3 — AI Interpretation workspace), rather than extending it.
-- The right fix is consolidation into the existing generic surface, not
-- maintaining a second narrower one. Purely additive removal: every other
-- admin_read branch is copied verbatim, unchanged, in its original order.
--
...
DROP FUNCTION IF EXISTS security.tuition_ai_control_read_impl();
