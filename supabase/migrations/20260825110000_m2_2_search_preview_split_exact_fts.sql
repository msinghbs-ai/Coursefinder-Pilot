-- CF-CHG-20260825-033
-- Exact lookup and FTS are deliberately separate so deterministic identity lookup is not mixed into semantic/relevance ranking.

create index if not exists course_documents_course_code_lower_idx on search.course_documents (lower(course_code)) where course_code is not null;
create index if not exists course_documents_stable_key_lower_idx on search.course_documents (lower(course_stable_key)) where course_stable_key is not null;
create index if not exists course_documents_title_lower_idx on search.course_documents (lower(course_title));
create index if not exists course_documents_provider_name_lower_idx on search.course_documents (lower(provider_name));
create index if not exists course_documents_provider_stable_key_idx on search.course_documents (provider_stable_key) where provider_stable_key is not null;
...
as $function$
with q as (select nullif(trim(p_identifier),'') v), hit as (
 select d.*,case when lower(d.course_code)=lower((select v from q)) then 'exact_course_code' else 'exact_course_id' end match_type
 from search.course_documents d
 where (select v from q) is not null
   and (lower(d.course_code)=lower((select v from q)) or lower(d.course_stable_key)=lower((select v from q)))
 order by case when lower(d.course_code)=lower((select v from q)) then 0 else 1 end, d.course_stable_key
 limit 1
...
with params as (
 ...
 select d.*,case when (select q from params)='' then 0::real else ts_rank_cd(d.search_tsv,(select tsq from params))::real end score
 from search.course_documents d
 where (p_country_codes is null or d.country_code=any(p_country_codes))
   ...
   and ((select q from params)='' or d.search_tsv @@ (select tsq from params))
), paged as (
 select * from ranked
...
select jsonb_build_object(
 ...
 'items',coalesce(jsonb_agg(jsonb_build_object(
   ...
   'visibility',jsonb_build_object('publication_status',publication_status),
   'freshness',jsonb_build_object('source_updated_at',source_updated_at,'generated_at',generated_at),
   'match',jsonb_build_object('mode','fts','keyword_score',score)
 ) order by case when (select sort_code from params)='relevance' then score end desc,
            case when (select sort_code from params)='title' then course_title end asc,
            case when (select sort_code from params)='provider_annual_tuition_asc' then provider_annual_tuition_amount end asc nulls last,
            case when (select sort_code from params)='provider_annual_tuition_desc' then provider_annual_tuition_amount end desc nulls last,
