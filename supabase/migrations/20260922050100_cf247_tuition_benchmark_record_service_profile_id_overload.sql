-- CF-CHG-20260915-247
-- Multi-model support foundation: new 12-argument overload keyed by
-- p_profile_id instead of a hardcoded profile code, so the benchmark
-- worker can record a qualifying run against whichever profile it just
-- fetched. Looks up and updates only that specific profile row (FOR
-- UPDATE), never any other. Existing 10- and 11-argument overloads are
-- untouched and remain in place, unused going forward once the worker
-- always supplies p_profile_id.
--
