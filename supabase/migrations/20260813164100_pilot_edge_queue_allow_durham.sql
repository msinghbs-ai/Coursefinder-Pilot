...
declare v_key text; v_id bigint; begin
 if p_function not in ('layer1-ca-on-college-programs','layer1-ca-algonquin-catalogue','layer1-ca-conestoga-catalogue','layer1-ca-fanshawe-pgwp','layer1-ca-mohawk-catalogue','layer1-ca-durham-programs') then raise exception 'Pilot Edge function is not allowlisted'; end if;
...
begin
 if new.function_name not in ('layer1-ca-on-college-programs','layer1-ca-algonquin-catalogue','layer1-ca-conestoga-catalogue','layer1-ca-fanshawe-pgwp','layer1-ca-mohawk-catalogue','layer1-ca-durham-programs') then new.status:='failed'; new.error_text:='function not allowlisted'; return new; end if;
