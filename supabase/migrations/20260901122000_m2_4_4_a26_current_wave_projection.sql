-- CF-CHG-20260830-048
-- M2.4.4 A26: parent progress uses the currently linked production grain.
-- Missing-URL inventory and superseded/cancelled corrective history are separate.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'child_batches',coalesce(cur.child_batches,0),
    'active_batches',coalesce(cur.active_batches,0),
    'cancelled_batches',coalesce(hist.cancelled_batches,0),
    'processed_items',r.completed_items+r.failed_items,
    'resolved_l2',coalesce(cur.resolved_l2,0),
    'escalated_l3',coalesce(cur.escalated_l3,0),
    'blocked',coalesce(cur.blocked,0),
    'vendor_units',coalesce(cur.vendor_units,0),
    'vendor_cost_usd',coalesce(cur.vendor_cost_usd,0),
    'heartbeat_at',cur.heartbeat_at,
    'historical_cancelled_processed',coalesce(hist.historical_cancelled_processed,0),
    'child_jobs',coalesce(cur.child_jobs,0),
    'evidence_count',coalesce(cur.evidence_count,0),
    'latest_evidence_at',cur.latest_evidence_at
