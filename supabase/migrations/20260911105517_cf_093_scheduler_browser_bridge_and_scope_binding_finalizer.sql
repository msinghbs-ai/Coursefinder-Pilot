...
begin
  ...
  if v_match[1] ~ '^[0-9.]+$' then
    ...
    foreach v_part in array v_parts loop
      if v_part !~ '^[0-9]{1,3}$' then return null; end if;
...
as $function$
  with scoped_base as materialized (
    ...
    select profile_id,course_id,source_url,current_version_id,
    ...
    select count(*) filter (where source_url is not null)::integer queueable_count,
           ...
           coalesce(md5(string_agg(
             ...
             case when source_url is not null then source_url else '<discover:'||coalesce(discovery_target,'')||'>' end,
             '|' order by profile_id,current_version_id,course_id,coalesce(source_url,'<discover:'||coalesce(discovery_target,'')||'>')
...
-- The public wrapper is SECURITY INVOKER; authenticated callers therefore require
-- EXECUTE on this independently rank-gated private bridge. Anonymous/public remain denied.
