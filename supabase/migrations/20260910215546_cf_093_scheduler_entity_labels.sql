...
-- CF-CHG-20260910-093
-- Codex P2: entity-scoped schedules must resolve canonical business labels rather
-- than displaying only an entity type. The helper is private to security and the
-- browser-facing list wrapper remains SECURITY INVOKER/rank-gated through its bridge.

create or replace function security.scheduler_entity_display(p_entity_type text,p_entity_id uuid)
returns text
language plpgsql
stable
security definer
...
begin
  ...
  case lower(coalesce(p_entity_type,''))
    when 'provider' then
      select coalesce(nullif(trim(p.display_name),''),nullif(trim(p.canonical_name),'')) into v_label
      from catalogue.providers p where p.id=p_entity_id;
    when 'course' then
      select coalesce(nullif(trim(c.display_title),''),nullif(trim(c.canonical_title),''),nullif(trim(c.course_code),'')) into v_label
      from catalogue.courses c where c.id=p_entity_id;
    when 'campus' then
      select nullif(trim(c.name),'') into v_label
      from catalogue.campuses c where c.id=p_entity_id;
    when 'scholarship' then
      select nullif(trim(s.name),'') into v_label
      from scholarship.scholarships s where s.id=p_entity_id;
    else
      v_label:=null;
  end case;
  return v_label;
...
revoke all on function security.scheduler_entity_display(text,uuid) from public,anon,authenticated;
grant execute on function security.scheduler_entity_display(text,uuid) to service_role;
      ...
      from (
        select p.id,p.country_code,p.layer,p.source_id,p.source_profile_id,p.entity_type,p.entity_id,
               ...
               coalesce(s.label,ps.label,lp.profile_key,
                 security.scheduler_entity_display(p.entity_type,p.entity_id),
