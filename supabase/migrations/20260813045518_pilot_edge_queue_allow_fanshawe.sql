...
begin
  if p_function is null or p_function not in ('layer1-ca-on-college-programs','layer1-ca-algonquin-catalogue','layer1-ca-conestoga-catalogue','layer1-ca-fanshawe-pgwp') then
  ...
  select net.http_post(url:='https://fxcwkweaxjtknorudmwp.supabase.co/functions/v1/'||p_function,
    headers:=jsonb_build_object('content-type','application/json','x-cf-pilot-key',v_key),body:=coalesce(p_body,'{}'::jsonb),timeout_milliseconds:=30000) into v_id;
  return v_id;
end $$;
...
begin
 if new.function_name not in ('layer1-ca-on-college-programs','layer1-ca-algonquin-catalogue','layer1-ca-conestoga-catalogue','layer1-ca-fanshawe-pgwp') then new.status:='failed';new.error_text:='function not allowlisted';return new;end if;
 begin new.net_request_id:=pipeline.svc_pilot_invoke_edge(new.function_name,new.body);new.status:='submitted';new.submitted_at:=now();
 exception when others then new.status:='failed';new.error_text:=sqlerrm;end;
 return new;
end $$;