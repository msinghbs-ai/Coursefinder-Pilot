...
create or replace function public.layer2_operator_sync(p_action text, p_profile_id uuid, p_limit integer default null)
returns jsonb
language plpgsql
security definer
...
as $function$
declare
  v_actor uuid:=auth.uid(); v_rank integer:=0; v_profile pipeline.layer2_source_profiles%rowtype;
  v_source pipeline.sources%rowtype; v_country text; v_provider_name text; v_catalogue integer:=0;
  v_queueable integer:=0; v_active uuid; v_batch_size integer:=10; v_items jsonb; v_batch uuid;
  v_req bigint; v_limit integer;
begin
  ...
  where ur.user_id=v_actor and (ur.expires_at is null or ur.expires_at>now()) and r.status='active';
  ...
  if v_profile.domain='course_facts' and v_source.provider_id is not null then
    ...
    select count(distinct d.course_id) into v_queueable from pipeline.layer2_course_discovery_candidates d
    where d.source_profile_version_id=v_profile.current_version_id and d.selected and d.discovered_url is not null;
  end if;
  ...
  if p_action='sync' then
    ...
    v_batch:=public.layer2_run_batch_create(p_profile_id,'manual',v_actor,v_items); v_req:=public.layer2_run_batch_dispatch(v_batch);
...
$function$;
revoke all on function public.layer2_operator_sync(text,uuid,integer) from public,anon;
grant execute on function public.layer2_operator_sync(text,uuid,integer) to authenticated;