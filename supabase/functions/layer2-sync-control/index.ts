...
if(['options','scope_options_page','preview_scope','start_scope','qualify_scope','preview_wave','start_wave','continue_wave','preview_background','start_background','policy','update_policy'].includes(action)){
 ...
 if(action==='options'){
  const[{data,error},{data:policy,error:pe}]=await Promise.all([
   svc.rpc('layer2_scope_countries_service',{p_actor:String(ctx.user_id)}),
   svc.rpc('layer2_execution_policy_service',{p_actor:String(ctx.user_id),p_action:'read',p_patch:{}})
  ]);
  if(error)return J(req,error.code==='42501'?403:400,{error:error.message});
  if(pe)return J(req,pe.code==='42501'?403:400,{error:pe.message});
  return J(req,200,{...(data||{countries:[]}),execution_policy:policy});
 }
 if(action==='policy'||action==='update_policy'){
  const{data,error}=await svc.rpc('layer2_execution_policy_service',{p_actor:String(ctx.user_id),p_action:action==='update_policy'?'update':'read',p_patch:b?.patch||{}});
  if(error)return J(req,error.code==='42501'?403:400,{error:error.message});return J(req,200,data||{});
 }
 if(action==='continue_wave'){
  const requestId=String(b?.request_id||'').trim();
  if(!requestId)return J(req,400,{error:'request_id_required'});
  const{data,error}=await svc.rpc('layer2_wave_scope_service',{p_actor:String(ctx.user_id),p_action:'continue',p_country_code:String(b?.country_code||'AU'),p_scope_type:String(b?.scope_type||'country'),p_scope_id:String(b?.scope_id||'').trim()||null,p_wave_size:Number(b?.wave_size||500),p_schedule_remaining:Boolean(b?.schedule_remaining),p_route_mode:String(b?.route_mode||'managed'),p_request_id:requestId});
 ...
 if(action==='preview_background'||action==='start_background'){
  const{data,error}=await svc.rpc('layer2_background_scope_service',{p_actor:String(ctx.user_id),p_action:action==='preview_background'?'preview':'start',p_country_code:country,p_scope_type:scopeType,p_scope_id:scopeId});
  if(error)return J(req,error.code==='42501'?403:400,{error:error.message});return J(req,200,data||{ok:true});
 }
 if(action==='preview_scope'||action==='qualify_scope'){
  const{data,error}=await svc.rpc('layer2_scale_scope_service',{p_actor:String(ctx.user_id),p_action:action==='preview_scope'?'preview':'qualify_wave',p_country_code:country,p_scope_type:scopeType,p_scope_id:scopeId,p_wave_size:action==='qualify_scope'?0:Math.min(Math.max(Number(b?.wave_size||0),0),500),p_sample_size:action==='qualify_scope'?0:Math.min(Math.max(Number(b?.sample_size||0),0),50)});
  if(error)return J(req,error.code==='42501'?403:400,{error:error.message});return J(req,200,data||{ok:true});
 }
 if(action==='preview_wave'||action==='start_wave'){
  const{data,error}=await svc.rpc('layer2_wave_scope_service',{p_actor:String(ctx.user_id),p_action:action==='preview_wave'?'preview':'start',p_country_code:country,p_scope_type:scopeType,p_scope_id:scopeId,p_wave_size:Math.min(Math.max(Number(b?.wave_size||500),1),5000),p_schedule_remaining:Boolean(b?.schedule_remaining),p_route_mode:String(b?.route_mode||'managed'),p_request_id:null});
  if(error)return J(req,error.code==='42501'?403:400,{error:error.message});return J(req,200,data||{ok:true});
 }
 const{data,error}=await svc.rpc('layer2_operator_scope_service',{p_actor:String(ctx.user_id),p_action:'start',p_country_code:country,p_scope_type:scopeType,p_scope_id:scopeId});
 if(error)return J(req,error.code==='42501'?403:400,{error:error.message});return J(req,200,data||{ok:true});
}
