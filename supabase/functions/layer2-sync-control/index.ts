...
if(['options','scope_options_page','preview_scope','start_scope','qualify_scope','preview_wave','start_wave','continue_wave'].includes(action)){
 ...
 if(action==='preview_scope'||action==='qualify_scope'){
  const{data,error}=await svc.rpc('layer2_scale_scope_service',{p_actor:String(ctx.user_id),p_action:action==='preview_scope'?'preview':'qualify_wave',p_country_code:country,p_scope_type:scopeType,p_scope_id:scopeId,p_wave_size:Math.min(Math.max(Number(b?.wave_size||5),1),10),p_sample_size:Math.min(Math.max(Number(b?.sample_size||10),1),20)});
  if(error)return J(req,error.code==='42501'?403:400,{error:error.message});return J(req,200,data||{ok:true});
 }
