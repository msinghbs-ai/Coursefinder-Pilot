...
const FN="layer3-source-pattern-benchmark",VERSION="layer3-source-pattern-benchmark-v1.1.0";
...
async function callModel(profile:any,key:string,caseName:string,sourceUrl:string,links:any[],negative=false){
 const prompt=[`Task class: source_pattern`,`Case: ${caseName}`,`Governed first-party host: ${new URL(sourceUrl).hostname}`,"Choose at most one Course/programme catalogue or discovery entrypoint from the supplied Evidence links.","The candidate URL MUST appear exactly in the supplied links and MUST stay on the governed host.","Do not infer Course identity, regulatory codes, fees, admissions, Search actions or Publication actions.",negative?"This is a negative control. No Course/programme catalogue link is present, so candidate_value must be null.":"","Return exactly: {\"candidate_value\":null OR \"https://...\", \"confidence\":0..1, \"rationale\":\"...\", \"evidence_quotes\":[\"...\"]}.","Evidence links:",...links.slice(0,60).map((x:any)=>`- ${x.text||"(no text)"} :: ${x.url}`)].filter(Boolean).join("\n");
 let last:any=null,totalCalls=0,input=0,output=0,cost=0,maxLatency=0;const returnedModels=new Set<string>(),attemptTrace:any[]=[];const attempts=Math.max(1,Math.min(Number(profile.retry_ceiling||0)+1,3));
 for(let attempt=0;attempt<attempts;attempt++){const ctl=new AbortController(),tm=setTimeout(()=>ctl.abort(),Number(profile.timeout_ms||30000)),st=performance.now();totalCalls++;try{const res=await fetch(String(profile.base_url).replace(/\/$/,"")+"/chat/completions",{method:"POST",signal:ctl.signal,headers:{"Authorization":"Bearer "+key,"Content-Type":"application/json","HTTP-Referer":"https://coursefinder.app","X-Title":"CourseFinder Source Pattern Benchmark"},body:JSON.stringify({model:profile.model_identifier,temperature:0,seed:0,max_tokens:Number(profile.max_output_tokens||1200),reasoning:{effort:"none",exclude:true},response_format:{type:"json_schema",json_schema:{name:"coursefinder_source_pattern",strict:true,schema:{type:"object",additionalProperties:false,required:["candidate_value","confidence","rationale","evidence_quotes"],properties:{candidate_value:{type:["string","null"]},confidence:{type:"number",minimum:0,maximum:1},rationale:{type:"string"},evidence_quotes:{type:"array",items:{type:"string"}}}}}},messages:[{role:"system",content:profile.prompt_system},{role:"user",content:prompt}]})});const payload=await res.json().catch(()=>({})),latency=Math.round(performance.now()-st);input+=Number(payload?.usage?.prompt_tokens||0);output+=Number(payload?.usage?.completion_tokens||0);cost+=Number(payload?.usage?.cost||0);maxLatency=Math.max(maxLatency,latency);if(payload?.model)returnedModels.add(String(payload.model));if(!res.ok){attemptTrace.push({attempt:attempt+1,http_status:res.status,latency_ms:latency,outcome:"provider_error"});last={payload,parsed:null,parse_error:`aggregator ${res.status}`,latency_ms:latency};continue}let parsed:any=null,parse_error:string|null=null;try{parsed=parseContent(payload?.choices?.[0]?.message?.content)}catch(e:any){parse_error=String(e.message||e)}attemptTrace.push({attempt:attempt+1,http_status:res.status,latency_ms:latency,outcome:parsed&&!parse_error?"structured_output":"malformed_output",response_model:payload?.model||null});last={payload,parsed,parse_error,latency_ms:latency};if(parsed&&!parse_error)break}catch(e:any){const latency=Math.round(performance.now()-st);maxLatency=Math.max(maxLatency,latency);attemptTrace.push({attempt:attempt+1,http_status:null,latency_ms:latency,outcome:"network_or_timeout",error:String(e?.name||"error")});last={payload:{},parsed:null,parse_error:String(e?.message||e),latency_ms:latency}}finally{clearTimeout(tm)}}return {...last,metrics:{external_calls:totalCalls,input_tokens:input,output_tokens:output,estimated_cost_usd:cost,max_latency_ms:maxLatency,returned_models:[...returnedModels],retry_count:Math.max(totalCalls-1,0),attempt_trace:attemptTrace}};
}
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  for(const ev of evs||[]){
   ...
   calls+=Number(result.metrics?.external_calls||0);input+=Number(result.metrics?.input_tokens||0);output+=Number(result.metrics?.output_tokens||0);cost+=Number(result.metrics?.estimated_cost_usd||0);maxLatency=Math.max(maxLatency,Number(result.metrics?.max_latency_ms||result.latency_ms||0));for(const m of result.metrics?.returned_models||[])returned.add(String(m));
   const val=validate(result.parsed,links,new URL(ev.source_url).hostname,null,false);
   providerCases.push({evidence_id:ev.id,source_url:ev.source_url,link_count:links.length,valid:val.valid,errors:[...val.errors,...(result.parse_error?[result.parse_error]:[])],candidate_value:val.candidate_value,confidence:val.confidence,model:result.payload?.model||null,latency_ms:result.latency_ms,retry_count:result.metrics?.retry_count||0,attempt_trace:result.metrics?.attempt_trace||[]});
  ...
  for(const c of synth){
   ...
   controlCases.push({case:c.name,valid:val.valid&&!result.parse_error,errors:[...val.errors,...(result.parse_error?[result.parse_error]:[])],candidate_value:val.candidate_value,confidence:val.confidence,model:result.payload?.model||null,latency_ms:result.latency_ms,retry_count:result.metrics?.retry_count||0,attempt_trace:result.metrics?.attempt_trace||[]});
  }
