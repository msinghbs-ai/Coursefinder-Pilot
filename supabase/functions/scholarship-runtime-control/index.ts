...
Deno.serve(async(req:Request)=>{
 ...
 if(Number(ctx?.role_rank||0)<4)return J(403,{error:"pipeline_operator_role_required"});
 ...
 if(!["preview","start"].includes(action))return J(400,{error:"unsupported_action"});if(!["country","university"].includes(scopeType))return J(400,{error:"scope_type_must_be_country_or_university"});if(scopeType==="university"&&!scopeId)return J(400,{error:"university_scope_id_required"});
