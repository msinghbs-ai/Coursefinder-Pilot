...
Deno.serve(async(req:Request)=>{
 ...
 try{
   ...
   if(action==="validate"){
     await svc.schema("ranking").from("manual_imports").update({
       status:"validated",
       validation_summary:{candidate_observations:out.candidateObservations,unknown_rank_semantics:out.unknownRankSemantics,indicator_cells:out.indicatorCells,reconciliation_preview:out.reconciliationPreview,worker_version:out.workerVersion,validated_at:new Date().toISOString()},
       parse_summary:{candidate_observations:out.candidateObservations,filename:out.filename,acquisition_mode:out.acquisitionMode},
