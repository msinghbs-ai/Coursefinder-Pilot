...
async function extractChain(sb:string,key:string,svc:any,jobId:string,evidenceId:string){
 const ec=await rpc(svc,"scholarship_detail_extraction_context",{p_job_id:jobId,p_evidence_id:evidenceId});
 ...
 if(!normalizedId){if(!ec?.attempt_id)throw Error("detail evidence has no acquisition attempt_id");const norm=await postJson(`${sb}/functions/v1/layer2-extract-v2`,key,{attempt_id:ec.attempt_id});normalizedId=String(norm?.normalized_evidence_id||"");if(!normalizedId)throw Error("normalisation returned no normalized_evidence_id")}
...
Deno.serve(async(req:Request)=>{
 ...
 const extractOnly=ctx?.payload?.extract_only===true||String(ctx?.payload?.extract_only||"")==="true";const priorEvidence=String(ctx?.payload?.source_evidence_id||ctx?.result?.source_evidence_id||ctx?.result?.evidence_id||"");
 ...
 try{
  if(extractOnly){if(!priorEvidence)throw Error("extract_only job has no source_evidence_id");const extraction=await extractChain(sb,key,svc,jobId,priorEvidence);const result={source_evidence_id:priorEvidence,extraction,publication_changed:false,canonical_mutation_authorised:false};await rpc(svc,"scholarship_scope_job_mark",{p_job_id:jobId,p_status:"succeeded",p_result:result,p_error:null,p_execution:execution});return J(200,{ok:true,job_id:jobId,...result})}
  const ar=await postJson(`${sb}/functions/v1/layer2-acquire-v2`,key,{profile_id:ctx.profile_id,target_url:ctx.target_url});let fanout:any=null;if(ar?.shared_fetch_id){try{fanout=await postJson(`${sb}/functions/v1/layer2-provider-page-fanout`,key,{shared_fetch_id:ar.shared_fetch_id})}catch(e:any){fanout={error:String(e.message)}}}
  let extraction:any=null;if(String(ctx?.payload?.acquisition_stage||"")==="first_party_detail"&&ar?.evidence_id)extraction=await extractChain(sb,key,svc,jobId,String(ar.evidence_id));
