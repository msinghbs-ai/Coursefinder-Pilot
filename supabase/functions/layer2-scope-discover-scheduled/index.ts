...
const FN="layer2-scope-discover-scheduled",BUCKET="evidence",VERSION="layer2-scope-discover-scheduled-v1.1.0";
...
function rankCandidates(course:any,html:string,base:string){const expectedTitle=clean(course.canonical_title||course.display_title),expectedCode=clean(course.course_code).toUpperCase(),esc=expectedCode.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),candidates=extractLinks(html,base).map((x:any)=>{const titleScore=similarity(expectedTitle,x.title||""),codeInTitle=Boolean(expectedCode)&&new RegExp(`\\b${esc}\\b`,"i").test(x.title||""),codeInUrl=Boolean(expectedCode)&&x.url.toUpperCase().includes(expectedCode),matchScore=Math.min(1,titleScore+(codeInTitle?.35:0)+(codeInUrl?.25:0));return{...x,title_score:Number(titleScore.toFixed(3)),regulatory_code_seen:Boolean(codeInTitle||codeInUrl),match_score:Number(matchScore.toFixed(3))}}).filter((x:any)=>x.match_score>=.35).sort((a:any,b:any)=>b.match_score-a.match_score).slice(0,20);let status="current_page_not_found",blocker:string|null="no plausible current first-party Course page found";if(candidates.length){const top=candidates[0],second=candidates[1];if(top.regulatory_code_seen&&top.match_score>=.8)status="exact_match";else if(top.match_score>=.75&&(!second||top.match_score-second.match_score>=.12))status="likely_match";else if(top.match_score>=.55)status="ambiguous";else status="identity_mismatch";blocker=status==="ambiguous"?"multiple plausible current Course pages":status==="identity_mismatch"?"current page candidate does not sufficiently match selected Course identity":null}return{expectedTitle,expectedCode,candidates,status,blocker,selected:["exact_match","likely_match"].includes(status)}}
Deno.serve(async(req:Request)=>{if(req.method!=="POST")return J(405,{ok:false,error:"POST required",workerVersion:VERSION});const sb=Deno.env.get("SUPABASE_URL")!,sk=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,svc=createClient(sb,sk,{auth:{persistSession:false}});try{
 ...
 const body=await req.json().catch(()=>({})),profileId=clean(body.profile_id),limit=Math.min(Math.max(Number(body.limit||10),1),50);if(!profileId)throw new Error("profile_id required");
 const ctx=await rpc(svc,"layer2_discovery_context",{p_profile_id:profileId,p_limit:limit}),rt=ctx.runtime,cfg=rt.configuration||{},courses=ctx.courses||[],actor=await rpc(svc,"layer2_automation_actor"),job=await rpc(svc,"layer2_prepare_job",{p_actor:actor,p_profile_id:profileId,p_job_type:"layer2_discovery"}),jobId=job.job_id;
 ...
 for(const course of courses){let acquired:any=null;try{
   ...
   const ranked=rankCandidates(course,acquired.html,target),rows:any[]=[];
