...
const FN="layer2-scope-discover-scheduled",BUCKET="evidence",VERSION="layer2-scope-discover-scheduled-v1.3.1";
...
async function acquireHtml(svc:any,rt:any,target:string,jobId:string,ua:string,cfg:any,budgetCapMs:number=90000,requireDiscoveryLinks:boolean=true){
 ...
 for(const route of (rt.routes||[])){
  ...
  try{
   ...
   return{attemptId,providerId:pc.id,providerKey:pc.provider_key,httpStatus:res.status,mime:"text/html",html,sourceUrl:target,screenshotUrl,latency,routePriority:route.priority,failures};
  }catch(e:any){
