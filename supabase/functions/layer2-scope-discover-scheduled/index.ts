...
const FN="layer2-scope-discover-scheduled",BUCKET="evidence",VERSION="layer2-scope-discover-scheduled-v1.3.2";
...
async function verifyCandidateDetail(svc:any,rt:any,cfg:any,course:any,ranked:any,jobId:string,ua:string,budgetMs:number){
 ...
 const raw=await rpc(svc,"layer2_evidence_capture",{p_source_id:rt.source_id,p_job_id:jobId,p_evidence_type:"layer2_html_snapshot",p_source_url:candidate.url,p_storage_path:rawPath,p_content_hash:rawHash,p_mime_type:"text/html",p_profile_version_id:rt.version_id,p_group_key:await digest(new TextEncoder().encode(`${rt.version_id}|${course.id}|identity-verification|${rawHash}`)),p_retention_class:"standard_365",p_retain_until:null,p_metadata:{layer:2,operation:"course_url_identity_verification",worker_version:VERSION,course_id:course.id,expected_course_code:expectedCode,provider_key:acquired.providerKey,detail_cricos_verified:verified,canonical_mutation_authorised:false}});
