...
const FN="layer2-scope-discover-scheduled",BUCKET="evidence",VERSION="layer2-scope-discover-scheduled-v1.3.6";
...
async function cleanupDuplicateUpload(c:any,ev:any,path:string){
 try{
  ...
  if(duplicate!==path||!retained||retained===path)return{removed:false,error:null};
  const rm=await c.storage.from(BUCKET).remove([path]);
  if(rm.error){console.warn("CF-055 duplicate Evidence cleanup failed",path,rm.error.message);return{removed:false,error:rm.error.message}}
  return{removed:true,error:null}
 }catch(e:any){console.warn("CF-055 duplicate Evidence cleanup failed",path,String(e?.message||e));return{removed:false,error:String(e?.message||e)}}
}
...
function discoveryUrl(cfg:any,course:any){const d=cfg.discovery_strategy||{};if(d.type==="first_party_search"&&d.search_url_template){const field=clean(d.query_field).toLowerCase();let raw="";if(field==="course_code")raw=clean(course.course_code);else if(field==="canonical_title")raw=clean(course.canonical_title||course.display_title);else if(field==="canonical_title_normalized")raw=norm(course.canonical_title||course.display_title);else raw=clean(`${course.course_code||""} ${course.canonical_title||course.display_title||""}`);if(!raw)throw new Error("profile discovery query field is empty");const q=encodeURIComponent(raw);return approvedUrl(String(d.search_url_template).replace("{query}",q),cfg)}const url=d.catalogue_url||cfg.discovery_url;if(!url)throw new Error("profile discovery URL missing");return approvedUrl(String(url),cfg)}
...
function usage(pc:any){return{vendor_units:pc?.provider_key==="direct-http"?0:1,vendor_units_basis:pc?.provider_key==="direct-http"?"internal_request":"provider_request_attempt",estimated_request_cost_usd:pc?.estimated_request_cost_usd??null}}
async function captureFirecrawlScreenshot(svc:any,rt:any,acquired:any,jobId:string,sourceEvidenceId:string,sourceUrl:string,scopeKey:string){
 if(acquired?.providerKey!=="firecrawl"||!acquired?.screenshotUrl)return{evidenceId:null,error:null};
 const u=U(acquired.screenshotUrl);if(!u||u.protocol!=="https:")return{evidenceId:null,error:"firecrawl_screenshot_url_invalid"};
 try{
  const ctl=new AbortController(),tm=setTimeout(()=>ctl.abort(),15000);let res:Response;
  try{res=await fetch(u.toString(),{redirect:"follow",signal:ctl.signal,headers:{"accept":"image/png,image/jpeg,image/webp"}})}finally{clearTimeout(tm)}
  if(!res.ok)return{evidenceId:null,error:`firecrawl_screenshot_http_${res.status}`};
  const mime=mimeOnly(res.headers.get("content-type"));if(!["image/png","image/jpeg","image/webp"].includes(mime))return{evidenceId:null,error:`firecrawl_screenshot_mime_${mime}`};
  const bytes=new Uint8Array(await res.arrayBuffer());if(!bytes.length)return{evidenceId:null,error:"firecrawl_screenshot_empty"};
  const h=await digest(bytes),ext=mime==="image/png"?"png":mime==="image/webp"?"webp":"jpg",path=`layer2/v2/discovery/${rt.profile_id}/${jobId}/${scopeKey}/screenshot.${ext}`;
  const up=await svc.storage.from(BUCKET).upload(path,bytes,{contentType:mime,upsert:false});if(up.error)return{evidenceId:null,error:"firecrawl_screenshot_upload_failed:"+up.error.message};
  const ev=await rpc(svc,"layer2_evidence_capture",{p_source_id:rt.source_id,p_job_id:jobId,p_evidence_type:"layer2_screenshot",p_source_url:sourceUrl,p_storage_path:path,p_content_hash:h,p_mime_type:mime,p_profile_version_id:rt.version_id,p_group_key:await digest(new TextEncoder().encode(`${sourceEvidenceId}|firecrawl-screenshot|${h}`)),p_retention_class:"standard_365",p_retain_until:null,p_metadata:{layer:2,operation:"course_url_discovery_visual",worker_version:VERSION,provider_key:"firecrawl",attempt_id:acquired.attemptId,source_evidence_id:sourceEvidenceId,screenshot_origin:"firecrawl_scrape_format",visual_evidence:true,canonical_mutation_authorised:false}});await cleanupDuplicateUpload(svc,ev,path);
  return{evidenceId:String(ev.evidence_id),error:null};
 ...
 for(const route of (rt.routes||[])){
  ...
  try{
   ...
   if(String(pc.request_template?.response_adapter||"").toLowerCase()==="firecrawl_v2"){
     ...
     const d=parsed?.data||parsed;html=String(d?.html||"");screenshotUrl=clean(d?.screenshot||d?.screenshotUrl||d?.metadata?.screenshot)||null;
   }
   if(!html||!/<[a-z][\s\S]*>/i.test(html)){
     const reason="extraction_failed";
     await rpc(svc,"layer2_provider_attempt_finish",{p_attempt_id:attemptId,p_status:"extraction_failed",p_http_status:res.status,p_mime:mimeOnly(res.headers.get("content-type")),p_raw_evidence:null,p_html_evidence:null,p_screenshot_evidence:null,p_extraction_status:"discovery_html_missing",p_blocker:"provider response did not contain usable HTML",p_metrics:{operation:"scope_discovery",provider_key:pc.provider_key,...usage(pc),route_priority:route.priority,fallback_reason:reason,latency_ms:latency,worker_version:VERSION}});
   ...
   return{attemptId,providerId:pc.id,providerKey:pc.provider_key,httpStatus:res.status,mime:"text/html",html,sourceUrl:target,screenshotUrl,latency,routePriority:route.priority,failures,vendorUnits:pc.provider_key==="direct-http"?0:1,vendorUnitsBasis:pc.provider_key==="direct-http"?"internal_request":"provider_request_attempt",estimatedRequestCostUsd:pc.estimated_request_cost_usd??null};
  }catch(e:any){
...
function rankCandidates(course:any,html:string,base:string,cfg:any){const expectedTitle=clean(course.canonical_title||course.display_title),rankingExpectedTitle=candidateExpectedTitle(course,cfg),expectedCode=clean(course.course_code).toUpperCase(),esc=expectedCode.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),requiredPrefix=clean(cfg?.discovery_strategy?.require_url_prefix).toLowerCase(),preferExact=cfg?.discovery_strategy?.candidate_exact_title_preference===true,links=extractLinks(html,base).filter((x:any)=>{if(!requiredPrefix)return true;try{return new URL(x.url).pathname.toLowerCase().startsWith(requiredPrefix)}catch{return false}}),candidates=links.map((x:any)=>{const observedTitle=clean(x.title||""),nExpected=norm(rankingExpectedTitle),nObserved=norm(observedTitle),exactTitle=Boolean(nExpected&&nObserved&&nObserved===nExpected),prefixTitle=Boolean(nExpected&&nObserved&&nObserved.startsWith(nExpected+" ")&&nObserved.split(" ").length-nExpected.split(" ").length<=2),titleScore=exactTitle?1:prefixTitle?(preferExact?.9:1):similarity(rankingExpectedTitle,observedTitle),codeInTitle=Boolean(expectedCode)&&new RegExp(`\b${esc}\b`,"i").test(x.title||""),codeInUrl=Boolean(expectedCode)&&x.url.toUpperCase().includes(expectedCode),matchScore=Math.min(1,titleScore+(codeInTitle?.35:0)+(codeInUrl?.25:0));return{...x,title_score:Number(titleScore.toFixed(3)),title_exact:exactTitle,title_prefix:prefixTitle,regulatory_code_seen:Boolean(codeInTitle||codeInUrl),match_score:Number(matchScore.toFixed(3))}}).filter((x:any)=>x.match_score>=.35).sort((a:any,b:any)=>b.match_score-a.match_score).slice(0,20);let status="current_page_not_found",blocker:string|null="no plausible current first-party Course page found";if(candidates.length){const top=candidates[0],second=candidates[1];if(top.regulatory_code_seen&&top.match_score>=.8)status="exact_match";else if(top.match_score>=.75&&(!second||top.match_score-second.match_score>=.12))status="likely_match";else if(top.match_score>=.55)status="ambiguous";else status="identity_mismatch";blocker=status==="ambiguous"?"multiple plausible current Course pages":status==="identity_mismatch"?"current page candidate does not sufficiently match selected Course identity":null}return{expectedTitle,rankingExpectedTitle,expectedCode,candidates,status,blocker,selected:["exact_match","likely_match"].includes(status)}}

async function verifyCandidateDetail(svc:any,rt:any,cfg:any,course:any,ranked:any,jobId:string,ua:string,budgetMs:number){
 if(!ranked?.selected||!ranked?.candidates?.[0])return null;
 const candidate=ranked.candidates[0],acquired=await acquireHtml(svc,rt,candidate.url,jobId,ua,cfg,budgetMs,false);
 const expectedCode=clean(course.course_code).toUpperCase(),verified=Boolean(expectedCode)&&String(acquired.html||"").toUpperCase().includes(expectedCode);
 const bytes=new TextEncoder().encode(acquired.html),rawHash=await digest(bytes),rawPath=`layer2/v2/discovery/${rt.profile_id}/${jobId}/${course.id}/identity-verification.html`,up=await svc.storage.from(BUCKET).upload(rawPath,bytes,{contentType:"text/html",upsert:false});if(up.error)throw new Error(`verification evidence upload: ${up.error.message}`);
 const raw=await rpc(svc,"layer2_evidence_capture",{p_source_id:rt.source_id,p_job_id:jobId,p_evidence_type:"layer2_html_snapshot",p_source_url:candidate.url,p_storage_path:rawPath,p_content_hash:rawHash,p_mime_type:"text/html",p_profile_version_id:rt.version_id,p_group_key:await digest(new TextEncoder().encode(`${rt.version_id}|${course.id}|identity-verification|${rawHash}`)),p_retention_class:"standard_365",p_retain_until:null,p_metadata:{layer:2,operation:"course_url_identity_verification",worker_version:VERSION,course_id:course.id,expected_course_code:expectedCode,provider_key:acquired.providerKey,detail_cricos_verified:verified,canonical_mutation_authorised:false}});await cleanupDuplicateUpload(svc,raw,rawPath);
 const shot=await captureFirecrawlScreenshot(svc,rt,acquired,jobId,String(raw.evidence_id),candidate.url,`${course.id}/identity-verification`);
 await rpc(svc,"layer2_provider_attempt_finish",{p_attempt_id:acquired.attemptId,p_status:verified?"succeeded":"extraction_failed",p_http_status:acquired.httpStatus,p_mime:"text/html",p_raw_evidence:raw.evidence_id,p_html_evidence:raw.evidence_id,p_screenshot_evidence:shot.evidenceId,p_extraction_status:verified?"detail_cricos_verified":"detail_cricos_missing",p_blocker:verified?null:`expected CRICOS ${expectedCode} not present on current first-party Course page`,p_metrics:{operation:"course_url_identity_verification",expected_course_code:expectedCode,detail_cricos_verified:verified,provider_key:acquired.providerKey,vendor_units:acquired.vendorUnits,vendor_units_basis:acquired.vendorUnitsBasis,estimated_request_cost_usd:acquired.estimatedRequestCostUsd,route_priority:acquired.routePriority,latency_ms:acquired.latency,screenshot_evidence_id:shot.evidenceId||null,screenshot_capture_error:shot.error||null,worker_version:VERSION}});
 return{verified,evidenceId:raw.evidence_id,screenshotEvidenceId:shot.evidenceId,attemptId:acquired.attemptId,providerKey:acquired.providerKey,sourceUrl:candidate.url};
}
...
Deno.serve(async(req:Request)=>{if(req.method!=="POST")return J(405,{ok:false,error:"POST required",workerVersion:VERSION});const sb=Deno.env.get("SUPABASE_URL")!,sk=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,svc=createClient(sb,sk,{auth:{persistSession:false}});try{
 ...
 for(const course of courses){const invocationRemainingMs=invocationBudgetMs-(performance.now()-invocationStarted);if(invocationRemainingMs<20000)break;let acquired:any=null;try{
   ...
   const nev=await rpc(svc,"layer2_evidence_capture",{p_source_id:rt.source_id,p_job_id:jobId,p_evidence_type:"layer2_extraction_input",p_source_url:target,p_storage_path:normPath,p_content_hash:normHash,p_mime_type:"application/json",p_profile_version_id:rt.version_id,p_group_key:await digest(new TextEncoder().encode(`${raw.evidence_id}|scope-discovery`)),p_retention_class:"standard_365",p_retain_until:null,p_metadata:{layer:2,operation:"course_url_discovery",worker_version:VERSION,course_id:course.id,source_evidence_id:raw.evidence_id,attempt_id:acquired.attemptId,provider_key:acquired.providerKey,canonical_mutation_authorised:false}});await cleanupDuplicateUpload(svc,nev,normPath);
   const ranked=rankCandidates(course,acquired.html,target,cfg),rows:any[]=[];let verification:any=null,verificationError:string|null=null;
   if(ranked.selected){try{const remain=Math.max(5000,invocationBudgetMs-(performance.now()-invocationStarted)-8000);verification=await verifyCandidateDetail(svc,rt,cfg,course,ranked,jobId,cfg.headers?.user_agent,remain)}catch(e:any){verificationError=String(e.message||e)}}
   const finalSelected=Boolean(ranked.selected&&verification?.verified),finalStatus=ranked.selected?(verificationError?"candidate":finalSelected?ranked.status:"identity_mismatch"):ranked.status,finalBlocker=ranked.selected?(verificationError?`candidate detail verification unavailable: ${verificationError.slice(0,500)}`:finalSelected?null:`current first-party Course page does not contain expected CRICOS ${ranked.expectedCode}`):ranked.blocker;
   if(ranked.candidates.length)ranked.candidates.forEach((x:any,i:number)=>rows.push({course_id:course.id,source_profile_version_id:rt.version_id,provider_attempt_id:i===0&&verification?.attemptId?verification.attemptId:acquired.attemptId,evidence_id:i===0&&verification?.evidenceId?verification.evidenceId:nev.evidence_id,discovered_url:x.url,discovered_title:x.title,discovered_regulatory_code:i===0&&finalSelected?ranked.expectedCode:(x.regulatory_code_seen?ranked.expectedCode:null),match_score:x.match_score,match_basis:{title_score:x.title_score,title_exact:x.title_exact,title_prefix:x.title_prefix,regulatory_code_seen:x.regulatory_code_seen,expected_title:ranked.expectedTitle,ranking_expected_title:ranked.rankingExpectedTitle,expected_course_code:ranked.expectedCode,worker_version:VERSION,provider_key:acquired.providerKey,detail_cricos_verified:i===0?Boolean(verification?.verified):false,verification_provider_key:i===0?verification?.providerKey||null:null,verification_evidence_id:i===0?verification?.evidenceId||null:null},status:i===0?finalStatus:"candidate",selected:i===0&&finalSelected,blocker:i===0?finalBlocker:null}));
   else rows.push({course_id:course.id,source_profile_version_id:rt.version_id,provider_attempt_id:acquired.attemptId,evidence_id:nev.evidence_id,status:"current_page_not_found",selected:false,match_basis:{expected_title:ranked.expectedTitle,ranking_expected_title:ranked.rankingExpectedTitle,expected_course_code:ranked.expectedCode,worker_version:VERSION,provider_key:acquired.providerKey,detail_cricos_verified:false},blocker:ranked.blocker});
   ...
   await rpc(svc,"layer2_provider_attempt_finish",{p_attempt_id:acquired.attemptId,p_status:"succeeded",p_http_status:acquired.httpStatus,p_mime:"text/html",p_raw_evidence:raw.evidence_id,p_html_evidence:raw.evidence_id,p_screenshot_evidence:discoveryShot.evidenceId,p_extraction_status:"discovery_evaluated",p_blocker:finalBlocker,p_metrics:{operation:"scope_discovery",candidate_count:ranked.candidates.length,selected:finalSelected,detail_cricos_verified:Boolean(verification?.verified),provider_key:acquired.providerKey,vendor_units:acquired.vendorUnits,vendor_units_basis:acquired.vendorUnitsBasis,estimated_request_cost_usd:acquired.estimatedRequestCostUsd,route_priority:acquired.routePriority,prior_failures:acquired.failures,latency_ms:acquired.latency,screenshot_evidence_id:discoveryShot.evidenceId||null,screenshot_capture_error:discoveryShot.error||null,worker_version:VERSION}});
   results.push({course_id:course.id,status:finalStatus,provider_key:acquired.providerKey,selected_url:finalSelected?ranked.candidates[0]?.url:null,candidates:ranked.candidates.length,detail_cricos_verified:Boolean(verification?.verified)});
 }catch(e:any){if(acquired?.attemptId)try{await rpc(svc,"layer2_provider_attempt_finish",{p_attempt_id:acquired.attemptId,p_status:"failed",p_http_status:null,p_mime:null,p_raw_evidence:null,p_html_evidence:null,p_screenshot_evidence:null,p_extraction_status:"blocked",p_blocker:String(e.message||e).slice(0,1000),p_metrics:{operation:"scope_discovery",provider_key:acquired?.providerKey||null,vendor_units:acquired?.vendorUnits??null,vendor_units_basis:acquired?.vendorUnitsBasis??null,estimated_request_cost_usd:acquired?.estimatedRequestCostUsd??null,worker_version:VERSION}})}catch{}results.push({course_id:course.id,status:"failed",error:String(e.message||e)})}
