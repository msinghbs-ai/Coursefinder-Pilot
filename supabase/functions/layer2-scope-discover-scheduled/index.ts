...
const FN="layer2-scope-discover-scheduled",BUCKET="evidence",VERSION="layer2-scope-discover-scheduled-v1.3.0";
...
async function verifyCandidateDetail(svc:any,rt:any,cfg:any,course:any,ranked:any,jobId:string,ua:string,budgetMs:number){
 ...
 await rpc(svc,"layer2_provider_attempt_finish",{p_attempt_id:acquired.attemptId,p_status:verified?"succeeded":"extraction_failed",p_http_status:acquired.httpStatus,p_mime:"text/html",p_raw_evidence:raw.evidence_id,p_html_evidence:raw.evidence_id,p_screenshot_evidence:null,p_extraction_status:verified?"detail_cricos_verified":"detail_cricos_missing",p_blocker:verified?null:`expected CRICOS ${expectedCode} not present on current first-party Course page`,p_metrics:{operation:"course_url_identity_verification",expected_course_code:expectedCode,detail_cricos_verified:verified,provider_key:acquired.providerKey,route_priority:acquired.routePriority,latency_ms:acquired.latency,worker_version:VERSION}});
 return{verified,evidenceId:raw.evidence_id,attemptId:acquired.attemptId,providerKey:acquired.providerKey,sourceUrl:candidate.url};
}
...
Deno.serve(async(req:Request)=>{if(req.method!=="POST")return J(405,{ok:false,error:"POST required",workerVersion:VERSION});const sb=Deno.env.get("SUPABASE_URL")!,sk=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,svc=createClient(sb,sk,{auth:{persistSession:false}});try{
 ...
 for(const course of courses){const invocationRemainingMs=invocationBudgetMs-(performance.now()-invocationStarted);if(invocationRemainingMs<20000)break;let acquired:any=null;try{
   ...
   await rpc(svc,"layer2_provider_attempt_finish",{p_attempt_id:acquired.attemptId,p_status:"succeeded",p_http_status:acquired.httpStatus,p_mime:"text/html",p_raw_evidence:raw.evidence_id,p_html_evidence:raw.evidence_id,p_screenshot_evidence:null,p_extraction_status:"discovery_evaluated",p_blocker:finalBlocker,p_metrics:{operation:"scope_discovery",candidate_count:ranked.candidates.length,selected:finalSelected,detail_cricos_verified:Boolean(verification?.verified),provider_key:acquired.providerKey,route_priority:acquired.routePriority,prior_failures:acquired.failures,latency_ms:acquired.latency,worker_version:VERSION}});
